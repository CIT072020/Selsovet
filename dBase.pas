...
//    function AdresPropisMen(DateFiks: TDateTime; strID: String): String;
    ...
    function GetSysIdBase(var sErr:String): String;
...
uses fMain, SelLibFR, fWarning, fsimpleD, fChoiceAdres, fNotLoadPunkt, fShablon, fChDir, mDRecInt, uBase;
...
//----------------------------------------------------------------
function TdmBase.GetSysIdBase(var sErr:String): String;
begin
  sErr:='';
  Result:='';
  if AdsConnection.IsConnected then begin
    ...
    try
      ...
      WorkQuery.Close;
      if Result='' then begin
        ConnectExecuteM('MERGE global ON (ID_BASE=0) '+
       	  'WHEN MATCHED     THEN  UPDATE SET sysid=NewIDString() '+
	        'WHEN NOT MATCHED THEN  INSERT ( ID_BASE, MAIN_BASE, SYSID) VALUES ( 0, true, NewIDString() ) ', sErr, 2);
        WorkQuery.Open;
        Result:=Trim(WorkQuery.Fields[0].AsString);
        WorkQuery.Close;
      end;
      ...
      on E:Exception do begin
        Result:='';
        sErr:='Ошибка создания идентификатора: '+E.Message;
      end;
    end;
  end;
end;
