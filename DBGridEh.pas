...
var
  NumNewColumn : Integer;  // номер новой колонки для события OnColExit  vadim 03.10.2003
...
function CheckHintTextRect(Grid: TCustomDBGridEh; RightIndent, FInterlinear: Integer;
  ws: String; ARect: TRect; WordWrap: Boolean; var TextWidth, TextHeight: Integer): Boolean;
var
  NewRect: TRect;
  uFormat: Integer;
begin
  Result := False;
  uFormat := DT_CALCRECT or DT_LEFT or DT_NOPREFIX or Grid.DrawTextBiDiModeFlagsReadingOnly;
  ...
  DrawTextEh(Grid.Canvas.Handle, ws, Length(ws), NewRect, uFormat);
...
begin
  ...
  if Canvas = nil then    // vadim  было:  if Canvas <> nil then
    ReleaseDC(0, DC);
...
{      J := Column.DefaultWidth;
    ...
    begin
      ...
      {try
        ...
        try
          if (Text='') or (Trim(Text)='.  .') or (Trim(Text)='.') then   // vadim 06.07.2012   было if (Text='') then
...
begin
  DrawState := Column.HighlightRequired and Grid.FDataLink.Active and
    (Grid.FDataLink.DataSet.State = dsInsert) and (Column.Field <> nil) and
...
begin
  ...
  FSortMarker := Value;
  Grid := FColumn.GetGrid;
  if Grid=nil then Exit;       // vadim  03.10.2003
...
//  было    FImageList.FreeNotification(GetGrid);
  end;
  if GetGrid <> nil then
    GetGrid.Invalidate;
end;
...
//-------------------------------------------------------------
//  if (Length(FColCellParamsEh.FText) = 0)          // vadim  03.10.2003
//    then Value := Column.DisplayText
//  else Value := FColCellParamsEh.FText;
//    Value := FColCellParamsEh.FText;
  Value := Column.DisplayText;    //было
  //--------------------------------------------------------------
...
procedure TCustomDBGridEh.PaintInplaceButton(Canvas: TCanvas; ButtonStyle: TEditButtonStyleEh;
...
//1.51    leftM := 0;
    ...
    WriteCellText {WriteTextEh}(DrawColumn, Canvas, Rect(drawRec1.Left-2, drawRec1.Top, drawRec1.Right, drawRec1.Bottom), not DrawTitleByThemes, 0,
...
//      InflateRect(DHRect, 1, 1);
  ...
  var
    ...
    DrawState: Boolean;
  begin
    DrawState := Column.HighlightRequired and IsCurrentRow(ARow) and
      (DataLink.DataSet.State = dsInsert) and (Column.Field <> nil) and
      Column.Field.Required and Column.Field.IsNull;
    if Assigned(Column.FOnCheckDrawRequiredState) then
      Column.FOnCheckDrawRequiredState(Self, Text, DrawState);
    if DrawState then
...
begin
  ...
  begin
    ...
    begin
      FInColExit := True;
      try
        NumNewColumn:=RawToDataColumn(RawCol);    //vadim    03.10.2003
        ColExit;
      finally
        FInColExit := False;
      end;
      //------------ vadim  03.10.2003 -----------------
      if NumNewColumn<>RawToDataColumn(RawCol) then begin
        if NumNewColumn < 0 then begin
          Exit;
        end else begin
          RawCol := DataToRawColumn(NumNewColumn);
        end;
      end;
      //----------------------------------------------
  ...
  begin
    ...
    begin
      ...
      if (Field.DataType = ftDate) then begin
        if (Trim(FEdittext)='.  .') or (Trim(FEdittext)='.')
          then FEditText:='';
        Column.UpdateDataValues(FEditText, FEditText, True);
      end else if (Field.DataType = ftFloat) then begin
        if Pos(' ',FEditText) > 0 
          then FEditText:=StringReplace(FEditText,' ','',[rfReplaceAll]);
        Column.UpdateDataValues(FEditText, FEditText, True);
      end else begin
          Column.UpdateDataValues(FEditText, FEditText, True);
      end;
      //---------------------------------------------------------
//  было    Column.UpdateDataValues(FEditText, FEditText, True);
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        CheckDrawRect := Rect(Where.Left, Where.Top, Where.Right+1, Where.Bottom+1);
...
begin
  ...
  begin
    ...
    begin
      ...
      if Flat then
        Dec(FilterRect.Left);
      PaintInplaceButton(Canvas, ebsDropDownEh, FilterRect,  ARect,
        0, False, Flat, DataLink.Active, Canvas.Brush.Color, nil);
      if FInplaceEditorButtonHeight < DefaultRowHeight then
...
begin
  ...
  begin
    ...
    if FIntMemTable.InstantReadCurRow = -1
      then NewRow := FixedRows
      else NewRow := FIntMemTable.InstantReadCurRow + TopDataOffset;
...
begin
  ...
  begin
    ...
    begin
      NewRow := FIntMemTable.InstantReadCurRow + TopDataOffset;
      if Row <> NewRow then
      begin
        if not (dgAlwaysShowEditor in Options) then HideEditor;
...
begin
  if ViewScroll then
    Result := FIntMemTable.InstantReadCurRow = DataRowNum
  else
...
procedure TCustomDBGridEh.OptimizeColsWidth(ColumnsList: TColumnsEhList);
  ...
  begin
    ...
    begin
      ...
      CheckHintTextRect(Self, RightIndent, 0, S, ARect, WordWrap, MaxColWidth, MaxTextHeight);
  ...
  begin
    ...
    begin
      ...
      CheckHintTextRect(Self, 0, 0, S, ARect, WordWrap,
  ...
  begin
    ...
    begin
      ...
      ARect := Rect(0,0,0,DefaultRowHeight);
      CheckHintTextRect(Self, 0, 0, S, ARect, Footer.WordWrap and FAllowWordWrap,
        CurColWidth, MaxTextHeight);
      CurColWidth := CurColWidth + ExtraWidth;
...
procedure TCustomDBGridEh.DrawTreeArea(Canvas: TCanvas; ARect: TRect);
  ...
  begin
    Canvas.FillRect(ARect);
    DrawTreeElement(Canvas, ARect, TreeElement, //False);
...
//          FBackground, NewAlignment, [], Value);
      ...
      if IsDataToolTips and (Column.GetColumnType in [ctKeyImageList, ctCheckboxes])
        then TextWider := True
        else TextWider := CheckHintTextRect(Self, RightIndent, FInterlinear,
