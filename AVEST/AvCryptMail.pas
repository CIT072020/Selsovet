...
procedure LoadAvCryptMailDLL(LibraryName: String; slLog:TStringList);
...
{  vadim
  !!! НЕОБХОДИТМО САМОСТОЯТЕЛЬНО ВЫЗЫВАТЬ   FreeLibrary(DLLHandle) см. uAvest.pas !!!
...
var
    W: DWORD;
    PA: PChar;
//    GetLongPathName:TFNGetLongPathName;
...
Begin
  ...
  if DLLHandle <> 0 Then begin
     ...
     if Assigned(GetLongPathName) then begin
      ...
      SetLength(Result, GetLongPathName(PA, @Result[1], MAX_PATH));
     end else begin
       Result := PA; // ExtractFilePath(PA);
     end;
...
var
       LN: String;
       Ver: String;
       sName,s:String; // vadim
       lErr:Boolean;
//       ldebug:Boolean;
  procedure AddToLog(sText:String);
  begin
    if (slLog<>nil) and (Trim(sText)<>'') then slLog.Add(sText);
  end;
begin
   if DLLLoaded then
      exit;
...
//   ldebug:=true;
   ...
   Ver := FileVersion(LN);
//   if lDebug then MessageBox(0,PAnsiChar('Загрузка '+LN),'',MB_OK + MB_ICONSTOP);
   AddToLog('загрузка '+LN);
   DLLHandle := LoadLibrary(PChar(LN));
//   if lDebug then MessageBox(0,PAnsiChar('Handle '+inttostr(DLLHandle)),'',MB_OK + MB_ICONSTOP);

{
    AvCmGetErrorInfo(nil, err, AVCM_SHORT_STRING, Nil, W, 0);
ALL res:=AvCmInit(AVCMF_STARTUP);
ALL res:=AvCmLogin(0, Nil, hDefSession, AVCMF_NO_AUTH);
ALL res:=AvCmLogout(hDefSession, 0);
ALL res:=AvCmOpenMsg(hSession, @sFile[1], Length(sFile), hMsg, AVCMF_IN_RAW_DATA);
ALL res:=AvCmMsgSign(hMsg, AVCMF_DETACHED+AVCMF_ADD_SIGN_CERT);
ALL AvCmGetMsg(hMsg, Nil, W, 0);
ALL AvCmCloseHandle(hMsg, 0);
ALL res:=AvCmSetMsgContent(hMsg, @sFile[1], Length(sFile), 0);
ALL AvCmGetMsgSign(hMsg, 0, hSign, 0);
ALL AvCmFindCertBySign(hSign, hCert, 0);
ALL res:=AvCmGetSignAttr(hSign, AVCM_SIGN_DATE_TIME, nil, @st1, W,  0);
ALL res:=AvCmGetCertAttr(hCert, AVCM_NOT_AFTER , nil, @st2, W, 0);
ALL res:=AvCmMsgVerifySign(hMsg, hCert, nPar);
ALL res:=AvCmOpenCertEnum(hSession, 1, @pare, hcert_enum, AVCMF_ALL_CERT);
ALL while AvCmEnumGet(hcert_enum, hCert, f) = 0 do begin
    res:=AvCmFindCrl(hSession, 1, @parc, hcrl, 0);
    res:=AvCmGetCrlAttr(hcrl, AVCM_NOT_AFTER, @st1, w, 0);  // $1B
    res:=AvCmCheckCrlDP(hSession, PANSIChar(sPath), 0)

}
   DLLLastError:='';
   lErr:=true;
   if DLLHandle >= 32 then  begin
      sName:=AvCryptMailDLLPath+'AvCryptMail.dll';  // vadim
      AddToLog(sName+', версия="'+Ver+'"');
      lErr:=false;
      DLLLoaded := true;
// vadim     SaveExit := ExitProc;
//      ExitProc := @NewExit;
      @AvCmInit := GetProcAddress(DLLHandle, 'AvCmInit');
      if not Assigned(AvCmInit) then begin
        s:=' Ошибка инициализации AvCryptMail.dll Отсутствует функция AvCmInit';
        AddToLog(s);
        MessageBox(0,PAnsiChar(s),'ОШИБКА',MB_OK + MB_ICONSTOP);
        lErr:=true;
      end;
      Assert(@AvCmInit <> nil, sName+': ошибка инициализации');
...
// было   Assert(@AvCmMsgOCSPAddResponse <> nil, sName+': отсутствует функция AvCmMsgOCSPAddResponse');
         if not Assigned(AvCmMsgOCSPAddResponse) then begin
           MessageBox(0,' Ошибка инициализации AvCryptMail.dll Отсутствует функция AvCmMsgOCSPAddResponse','ОШИБКА',MB_OK + MB_ICONSTOP);
  ...
  // 3.0.0
      AvCmCheckCrlDP:=nil;   // vadim
      s:='Отсутствует функия AvCmCheckCrlDP("Контроль точет распределения СОС")';
      if Ver > '3.0.0' then begin
         @AvCmCheckCrlDP := GetProcAddress(DLLHandle, 'AvCmCheckCrlDP');
        if not Assigned(AvCmCheckCrlDP) then begin
          AddToLog(s);
          MessageBox(0,PANSIChar(s),'ОШИБКА',MB_OK + MB_ICONSTOP);
          @AvCmCheckCrlDP := @_AvCmPrm4;
          lErr:=true;
        end;
//         Assert(@AvCmCheckCrlDP <> nil, sName+': отсутствует функция AvCmCheckCrlDP');
      end else begin
         @AvCmCheckCrlDP := @_AvCmPrm4;
         AddToLog(s);
      end;
  ...
  //3.3.3.910
      AvCmMsgSetAttribute:=nil;  // vadim
      if Ver > '3.3.3' then begin
         @AvCmMsgSetAttribute := GetProcAddress(DLLHandle, 'AvCmMsgSetAttribute');
         if not Assigned(AvCmMsgSetAttribute) then begin
           lErr:=true;
           MessageBox(0,' Ошибка инициализации AvCryptMail.dll Отсутствует функция AvCmMsgSetAttribute','ОШИБКА',MB_OK + MB_ICONSTOP);
           @AvCmMsgSetAttribute := @_AvCmPrm6;
         end;
//  !!!       Assert(@AvCmMsgSetAttribute <> nil, sName+': отсутствует функция AvCmMsgSetAttribute');
      end else begin
         @AvCmMsgSetAttribute := @_AvCmPrm6;
      end;
      ...
      if Ver > '3.4.0' then begin
         ...
         if not Assigned(AvCmCreateScep) then begin
           lErr:=true;
           MessageBox(0,' Ошибка инициализации AvCryptMail.dll Отсутствует функция AvCmCreateScep','ОШИБКА',MB_OK + MB_ICONSTOP);
           @AvCmCreateScep := @_AvCmPrm4;
         end;
// !!!        Assert(@AvCmCreateScep <> nil, sName+': отсутствует функция AvCmCreateScep');
      ...
      if lErr then begin
        DLLLastError:='Ошибка инициализации AvCryptMail.dll';
        AddToLog(DLLLastError);
      end;

   end else begin
     DLLLoaded := false;
//      if lDebug then MessageBox(0,PAnsiChar(inttostr(GetLastError())+' '+SysErrorMessage(GetLastError())),'ОШИБКА',MB_OK + MB_ICONSTOP);
     DLLLastError:=SysErrorMessage(GetLastError());
     AddToLog('ошибка загрузки: '+IntToStr(GetLastError())+' '+DLLLastError);
