...
uses Forms, SysUtils, Classes, adscnnct, adstable, frxClass, frxDBSet, NativeXML, FuncPr, QStrings,
...
const
  //-------- РОЖДЕНИЯ -----------
  arrSrcB = 'soato, ONA_SOATO, id_soato, id_soatom, DateR, POL, TYPEROD, GIVOROGD, MASSA, SCHET, DLINA, SP_A, SP_B, SP_C, SP_D, SP_E, SP_F, SP_G, SP_H, SP_I, SP_J, SP_K ' +
...
// 29.10.2015			'familia,name,otch,identif,on_identif,ona_identif,dokument,isprav,decl_id,adres';
  arrDstB = 'soato, soatm, id_soato, id_soatom, dateBirth, sex, typeBirth, living, massBody, orderBody, lengthBody, A, B, C, D, E, F, G, H, I, J, K, dateBirthFather, dateBirthMother, ' +
...
// 29.10.2015			 'surname,name,patronymic,id,id_father,id_mother,document,changes,rec_true,adres';
  // ------- СМЕРТИ --------------
  arrSrcD = 'soato, SOATO, id_soato, id_soatom, POL, NATION, name1, DATES, ONLYGOD, SMERT_POSL, DateR, ONLYGOD_R, SEM, OBRAZ, DOK_KOD, PR_OSN, PR_OSN_NAME, PR_NEP, PR_NEP_NAME, PR_MEST, PR_MEST_NAME,'+
            'lich_nomer,svid_nomer,datesv,dokument,adres,sdan_dok,decl_id, nomer';
  arrDstD = 'soato, soatm, id_soato, id_soatom, sex, nation, nationName, dateDeath, onlygod, deathFollowed, dateBirth, onlygod_r, maritalStatus, education, document, basicReasonM, basicReasonName, directReasonM, directReasonName, placeDeathsM, placeDeathsName,'+
			'id,svid_nomer,datesv,doc,soato_res,sdan_dok,rec_true, nomer';
  // ------- БРАКИ ---------------
  arrSrcM  = 'soato, id_soato, DATEZ, ON_DATER, ONA_DATER, ON_NATION, name, ONA_NATION, name2, ON_OBRAZ, ONA_OBRAZ, ON_SEM, ONA_SEM, ON_GRAG, name3, ONA_GRAG, name4, DETI_COUNT,'+
			 'on_identif,on_soato, id_on_soato, ona_identif,ona_soato,id_ona_soato,decl_id, m_adres, w_adres, nomer';
  arrDstM  = 'soato, id_soato, dateRegMarriage, mDateBirth, wDateBirth, mNation, mNationName, wNation, wNationName, mEducation, wEducation, mMaritalStatus, wMaritalStatus, mCitizenship, mCitizenshipName, wCitizenship, wCitizenshipName, countChild,'+
			 'm_id,m_soato,id_m_soato,w_id,w_soato,id_w_soato,rec_true, m_adres, w_adres, nomer';
// ------ РАЗВОДЫ --------------
  arrSrcDiv = 'soato, id_soato, DATEZ, ON_DATER, ONA_DATER, ON_NATION, name, ONA_NATION, name2, ON_OBRAZ, ONA_OBRAZ, '+
              'ON_SEM, ONA_SEM, BRAK_DATE, DETI_COUNT,'+
			  'on_identif,on_soato,id_on_soato,ona_identif,ona_soato,id_ona_soato,decl_id, sud_sila, m_adres, w_adres, nomer';
  arrDstDiv = 'soato, id_soato, dateTermMarriage, mDateBirth, wDateBirth, mNation, mNationName, wNation, wNationName, mEducation, ' +
              'wEducation, mCountMarriage, wCountMarriage, dateRegMarriage, countChild,'+ //, heAge, sheAge';
			  'm_id,m_soato,id_m_soato,w_id,w_soato,id_w_soato,rec_true, sud_sila, m_adres, w_adres, nomer';
implementation
  ...
  try
    ...
    for i := 0 to n do begin
        ...
        if arrDst[i] = 'dateBirth' then begin
         ...
         if arrDst[i] = 'id_soato' then begin
           script := script + ' ate1.ate_id as id_soato';
           flag := true;
         end;
         if arrDst[i] = 'id_soatom' then begin
           script := script + ' ate2.ate_id as id_soatom';
           flag := true;
         end;
...
begin
  ...
  try
    ...
    ' AND VOSSTAN=false ORDER BY ' + strOrder;
//    memowrite('!!!sr.sql',aqReadBase.SQL.Text);
    aqReadBase.ExecSQL;
    aqReadBase.Open;
    if aqReadBase.RecordCount > 0 then
      Result := aqReadBase
...
// id_soato, id_m_soato, id_w_soato
        if arrSrc[i] = 'id_soato' then begin
          script := script + ' ate1.ate_id as id_soato';
          flag := true;
        end;
        if arrSrc[i] = 'id_on_soato' then begin
          script := script + ' ate2.ate_id as id_m_soato';
          flag := true;
        end;
        if arrSrc[i] = 'id_ona_soato' then begin
          script := script + ' ate3.ate_id as id_w_soato';
          flag := true;
        end;
//--------------
        if not flag then begin
    ...
    ' AND VOSSTAN=false ORDER BY ' + strOrder;
//    memowrite('!!!br.sql',aqReadBase.SQL.Text);
    aqReadBase.ExecSQL;
    aqReadBase.Open;
    if aqReadBase.RecordCount > 0 then
      Result := aqReadBase
...
//--------------
    ...
    ' AND CONVERT(MONTH(ar.DATEZ), SQL_VARCHAR) = ' + '''' + parMonth + '''' +
    ' AND VOSSTAN=false ORDER BY ' + strOrder;
//    memowrite('!!!rb.sql',aqReadBase.SQL.Text);
    aqReadBase.ExecSQL;
    aqReadBase.Open;
    if aqReadBase.RecordCount > 0 then
      Result := aqReadBase
...
var
  ...
  procedure check_val();
  begin
    val:=sdUTF8EscapeString(val);
    Q_SpaceCompressInPlace(val);
  end; 
begin
