unit fEdLink2; //форма просмотра и редактирования связей с документами
interface
uses
...
function EditDocLink2(DBItem: TDBItem; DataSet: TDataSet; DataSource: TDataSource; ViewMode: TViewMode; DocType: Integer): Boolean;
...
begin
   Result:=False;
   with TfmEdLink2.Create(nil) do begin
...
procedure TfmEdLink2.SetDBItem(DBItem: TDBItem; DataSet: TDataSet; DataSource: TDataSource);
var
...
procedure TfmEdLink2.FormCreate(Sender: TObject);
begin
   FDBAware:=TDBAwareList.Create;
end;

procedure TfmEdLink2.FormDestroy(Sender: TObject);
begin
   FDBAware.Free;
end;

procedure TfmEdLink2.OnButtonClick(Sender: TObject; var Handled: Boolean);
begin
//
end;

procedure TfmEdLink2.dcInBaseChange(Sender: TObject);
var
   IN_BASE: Integer;
begin
...
procedure TfmEdLink2.UpdateLinkDoc(DocId: Integer);
begin
   quSelDoc.SQL.Text:=Format(
      'select d.SIGN_NAME as Q_SIGN_NAME, trim(sp1.PROC_NO)+'' ''+sp1.FULL_NAME as Q_ADM_PROC, d.CONTENT as Q_CONTENT, d.ANS_KIND as Q_ANS_KIND, d.ANS_DATE as Q_ANS_DATE, d.ANS_IND as Q_ANS_IND, d.ANS_TEXT as Q_ANS_TEXT from DOCMAIN d '+
      'left join SPRDOCADMPROC sp1 on sp1.ID=d.ADM_PROC '+
