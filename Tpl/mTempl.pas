...
type
   ...
   private
      ...
      FUserProp:String;
      // функция скрипта для определения доступности
      FScriptEnabled:String;
      //!!!!!!!!!!!!!
      FFields: string;
      //!!!!!!!!!!!!!
      FTables: string;
      ...
      FSubMenu: string;
      // список кодов СОАТО для отбора шаблонов, доступных для конкретного района, сельсовета
      FSoatoList: TStringList;
      // список идентификатор для отбора шаблонов, доступных для конкретного района
      FOrgList: TStringList;
      // уникальный идентифиактор шаблона
      // в списке шаблонов не может присутствовать два шаблона с одинаковым идентификатором
      FUniqueId: Integer;
   private
   ...
   public
      ...
      property UserProp: string read FUserProp;
      ...
      property ScriptEnabled: string read FScriptEnabled;
      ...
      property SoatoList: TStringList read FSoatoList;
      property OrgList: TStringList read FOrgList;
   ...
   public
      ...
      function GetByUniqueId(UniqueId: Integer): TTemplate;
   public
   ...
   TOnBeforeAddTemplateEvent=function (Template: TTemplate): Boolean of object;
   ...
   public
      ...
      function ReadTemplateList(TemplateList: TTemplateList; Params: TStringList; Soato: string; OrgId: Integer; OnBeforeAdd: TOnBeforeAddTemplateEvent): Boolean; virtual; abstract;
   end;
   ...
   private
      ...
      FParams: TStringList;
      // событие срабатывает перед добавлением шаблона в общий список
      //  если функция возвращает True, то переданный в качестве параметра шаблон
      //  добавляется в общий список
      FOnBeforeAddTemplate: TOnBeforeAddTemplateEvent;
      // текущий СОАТО (на основании этого СОАТО производится отбор шаблонов)
      FSoato: string;
      // текущий идентификатор организации (на основании этого идентификатора производится отбор шаблонов)
      FOrgId: Integer;
   public
      constructor Create;
      destructor Destroy; override;
   ...
   public
      ...
      property OnBeforeAddTemplate: TOnBeforeAddTemplateEvent read FOnBeforeAddTemplate write FOnBeforeAddTemplate;
      property Soato: string read FSoato write FSoato;
   ...
   public
      ...
      function ReadTemplateList(TemplateList: TTemplateList; Params: TStringList; Soato: string; OrgId: Integer; OnBeforeAdd: TOnBeforeAddTemplateEvent): Boolean; override;
   end;
...
begin
   ...
   FSoatoList:=TStringList.Create;
   FOrgList:=TStringList.Create;
   FUniqueId:=-1;
end;
...
function TTemplateList.GetByUniqueId(UniqueId: Integer): TTemplate;
var
   I: Integer;
   Item: TTemplate;
begin
   Result:=nil;
   if UniqueId<>-1 then begin
      for I:=0 to Pred(GetCount) do begin
         Item:=GetItem(I);
         if (Item<>nil) and (Item.UniqueId=UniqueId) then begin
            Result:=Item;
            Break;
         end;
      end;
...
begin
   ...
   if FTemplateReaderClass<>nil then begin
      ...
      try
         Result:=TemplateReader.ReadTemplateList(FTemplateList, FParams, FSoato, FOrgId, FOnBeforeAddTemplate);
      finally
         TemplateReader.Free;
      end;
   end;
   ...
   try
      ...
      if sl.Count>0 then begin
         ...
         if (Caption<>'') {and (TemplateText<>'') and (FormTemplate<>'') and (SQLText<>'')} then begin
            ...
            Result.FUserProp:=sl.Values['UserProp'];
            Result.FScriptEnabled:=sl.Values['Enabled'];
            Result.Params.Assign(TemplateParams);
            Result.UniqueId:=StrToIntDef(sl.Values['UniqueId'], -1);
            Result.SoatoList.CommaText:=sl.Values['Soato'];
            Result.OrgList.CommaText:=sl.Values['OrgId'];
         end;
      end;
   finally
      SQLText.Free;
...
function TReadTemplateFromIni.ReadTemplateList(TemplateList: TTemplateList; Params: TStringList; Soato: string; OrgId: Integer; OnBeforeAdd: TOnBeforeAddTemplateEvent): Boolean;
var
   I: Integer;
   J: Integer;
   S: string;
   ...
   procedure ParseFullName(FullName: string);
   var
      P: Integer;
   begin
      P:=Pos('.', sl[I]);
   ...
   begin
      fl:=False;
      if TemplateList.GetByUniqueId(Template.UniqueId)=nil then begin
         fl:=True;
         if Assigned(OnBeforeAdd) then begin
            fl:=OnBeforeAdd(Template);
         end;
      end;
      if fl then begin
...
begin
   if TemplateList<>nil then begin
      ...
      try
         ...
         TemplateList.Clear;
         // добавление шаблонов по идентификатору организации
         for I:=0 to Pred(sl.Count) do begin
            ParseFullName(sl[I]);
            Template:=ReadTemplate(TemplateGroup, TemplateName, Params);
            if (Template<>nil) and (OrgId>0) then begin
               if Template.OrgList.IndexOf(IntToStr(OrgId))<>-1 then begin
                  AddTemplate;
               end;
            end;
         end;
         // добавление шаблонов по СОАТО
         Soato:=Trim(Soato);
         for I:=0 to Pred(sl.Count) do begin
            ParseFullName(sl[I]);
            Template:=ReadTemplate(TemplateGroup, TemplateName, Params);
            if (Template<>nil) and (Soato<>'') then begin
               for J:=0 to Pred(Template.SoatoList.Count) do begin
                  S:=Trim(Template.SoatoList[J]);
                  if (S<>'') and (Length(S)<=Length(Soato)) and (Copy(Soato, 1, Length(S))=S) then begin
                     AddTemplate;
                     Break;
                  end;
               end;
         ...
         // добавление оставшихся шаблонов
         for I:=0 to Pred(sl.Count) do begin
            ParseFullName(sl[I]);
            Template:=ReadTemplate(TemplateGroup, TemplateName, Params);
         ...
         while CurPos<=MaxLen do begin
            ...
            if Ch='#' then begin
               ...
               if Table=nil then begin
                  ...
                  if Parameter=nil
                    then Parameter:=ParameterList.ByCodeName(CodeName);
               end;
               if Parameter<>nil then begin
