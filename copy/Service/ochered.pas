...
begin
  ...
  while not q.Eof do begin
    ...
    for m:=0 to sl.Count-1 do begin
      ...
      END ELSE BEGIN
        if (((nType=0) and ((_resh.Tip=OCH_RESH_SN) or (_resh.Tip=OCH_RESH_POST) or (_resh.Tip=OCH_RESH_VOST) )) or (_resh.Tip=nType)) and (n<6) then begin
          curMen.FillReshOch(arrResh[n], -4, StrToInt(sl.Strings[m]));  // решение по AUTO_ID
          n:=n+1;
        end;
      END
...
PARAMS=PAR1=24,PAR2=IZV
    ...
    IF lVipiska THEN BEGIN //---------------- ВЫПИСКА
      if arrResh[1].Tip>-1 then begin
        ...
        if podpis_shtamp()<1 then begin
          if arrresh[1].Tip=OCH_RESH_SN then  sNomPril:='12' else if arrresh[1].Tip=OCH_RESH_RAZD then sNomPril:='6' else if arrresh[1].Tip=OCH_RESH_UNION then sNomPril:='7' else if arrresh[1].Tip=OCH_RESH_PERE then sNomPril:='8' else sNomPril:='4';
            ds.FieldByName('PRIL').AsString:='Приложение '+sNomPril+CRLF+'к постановлению '+CRLF+'Министерства жилищно-'+CRLF+'коммунального хозяйства'+CRLF+
                                             'Республики Беларусь'+CRLF+'27.12.2010 № 28';
        end;
        ds.Fld('RESH').AsString:=Och_GetStrResh(arrresh[1].Tip,arrresh[1].Otkaz);
        ...
        if arrResh[2].Tip=-1 then begin
          if not lIskl then sText:=sText+CRLF+'Номер очереди '+s+'.'
        end else begin
          if lIskl 
        ...
        for n:=2 to 5 do begin
          if arrResh[n].Tip>-1 then begin
            if lIskl then begin
              ...
              if s<>'' then sText:=CheckZnackDel(sText,'.')+', номер очереди '+s;
            end;
          end;
        end;
      end;
...
//       resh:=Men.GetReshOch(OCH_TYPE_OBSH, OCH_RESH_POST);
      ds.Fld('DATER').AsString := DatePropis(arrResh[1].DateR,6);
      ds.Fld('NOMER').AsString := arrResh[1].Nomer;
      if arrResh[1].TIP=OCH_RESH_SN then begin
        lAdd:=true;
        ds.Fld('NAME_IZV').AsString:='о снятии граждан с учета нуждающихся в улучшении жилищных условий';
        sText:='на основании '+arrResh[1].GetTextOsnov(nTipText,false)+'вы с семьей '+curMen.Fld('KOLVO_SOSTAV').AsString+' чел., '+sSam+','+
               curMen.SostavSemToStr('OTN;FIO;YEAR',', ')+' сняты с учета нуждающихся в улучшении жилищных условий по '+
               arrResh[1].NameList(2,'Д')+' с '+DatePropis(arrResh[1].DateR,6);
        for n:=2 to 5 do begin
          if arrResh[n].TIP>-1 then
            sText:=sText+' и по '+arrResh[n].NameList(2,'Д')+' с '+DatePropis(arrResh[n].DateR,6);
        end;
        sText:=sText+'.';
      end else if (arrResh[1].TIP=OCH_RESH_POST) or (arrResh[1].TIP=OCH_RESH_VOST) then begin
        lAdd:=true;
        if arrResh[1].TIP=OCH_RESH_POST then s:='поставлены на учет' else s:='восстановлены на учете';
        ...
        sText:='вы с семьей '+curMen.Fld('KOLVO_SOSTAV').AsString+' чел.: '+sSam+', '+curMen.SostavSemToStr('OTN;FIO;YEAR',', ')+s+
               ' граждан, нуждающихся в улучшении жилищных условий, и включены в '+arrResh[1].NameList(2, 'И')+' с '+DatePropis(arrResh[1].DateSilaResh,6);//DatePropis(curMen.Fld('DEC_DATE').AsDateTime,6);
        s:=curMen.NomerOch(arrResh[1].ochered_id,'RESH_ADD_NOMER');  // с учетом параметра
        if s<>'' then begin 
          if arrResh[1].TIP=-1 then sText:=sText+CRLF+'Номер очереди '+s+'.'
                               else sText:=sText+', номер очереди '+s;
        end;
        for n:=2 to 5 do begin
          if arrResh[n].TIP>-1 then begin
            sText:=sText+' и '+arrResh[n].NameList(2, 'И')+' с '+DatePropis(arrResh[n].DateSilaResh,6);
