...
begin
  ...
  f.AddParamEx(GlobalTask.GetLastValueAsString('N_PRIZ_UCH'), 'Номер призывного участка', 'PRIZ_UCH','WIDTH=120');
  f.AddParamEx(GlobalTask.GetLastValueAsInteger('ORIENT_LP'), 'Ориентация листа', 'ORIENT','TYPE=LIST~DESC=KEY_ORIENT~WIDTH=120');
  ...
  if (n=1) and (f.GetValue('DATE1','D')>0) and (f.GetValue('DATE2','D')>0) then begin
    ...
    GlobalTask.SetLastValueAsInteger('ORIENT_LP', f.GetValue('ORIENT','N'));
