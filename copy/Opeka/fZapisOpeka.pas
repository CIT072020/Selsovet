...
type
  ...
  TfmZapisOpeka = class(TfmSimpleD)
    ...
    TBItemStep2otm: TTBItem;
    TBItemStep2otst: TTBItem;
    edVid: TDBComboBoxEh;
    lbDateOsv: TLabel;
    edDateOsv: TDBDateTimeEditEh;
    Label61: TLabel;
    ...
    Label66: TLabel;
    edDateOtst: TDBDateTimeEditEh;
    Label70: TLabel;
    edNomerOtst: TDBEditEh;
    edPrich: TDBEditEh;
    edNomerOsv: TDBEditEh;
    ...
    procedure N_F_edDocOpekaOCheckDrawRequiredState(Sender: TObject;
      var DrawState: Boolean);
    procedure N_F_edDocOpekaCheckDrawRequiredState(Sender: TObject;
      var DrawState: Boolean);
  private
//    H : THintWindow;
...
//  colorResh:=$00FEF9E7;
  ...
  d2xml:=ds2xml.DS_Add(tbReshSud,false);
...
var
  ...
  lCheckDateGIS, lFullWrite : Boolean;
  arr:TArrStrings;
...
//???  if (DokumentTIP.AsString='') then    AddErr(' Сирота или без попечения. ');
  ...
  if (DokumentVID.AsString<>'') and  not DokumentDATER.IsNull then begin
  ...
  if not DokumentDATE_OTM.IsNull  and (dUst>0) and (dUst>DokumentDATE_OTM.AsDateTime) then AddErr(' Дата установления больше даты прекращения. ');
...
//  [KEY_GIS_SPR133]   4=решение   8=приказ
  ...
  if Result then  begin
    ...
    lCheckDateGIS:=CheckDateEditGIS;  // функция д.б. своя для каждого типа документа !!!
...
//    strField:=UpperCase(Field.FieldName);
    if Pos(Field.FieldName, 'DATEZ;DATE_OTM;DATE_OSV;DATE_OTST;NOMER_UST;NOMER_OTM;NOMER_OSV;NOMER_OTST;UST_ORG;OTM_ORG;UST_ORG_SPR;OTM_ORG_SPR;')>0 then begin
      if (Field.FieldName='DATE_OTM') then begin
        if (Field.AsString='') then DokumentNOMER_OTM.AsString:='';
//        edNomerOtm.Enabled:=not (Field.AsString='');
      end else if (Field.FieldName='DATE_OSV') then begin
        if (Field.AsString='') then DokumentNOMER_OSV.AsString:='';
//        edNomerOsv.Enabled:=not (Field.AsString='');
      end else if (Field.FieldName='DATE_OTST') then begin
        if (Field.AsString='') then DokumentNOMER_OTST.AsString:='';
//        edNomerOtst.Enabled:=not (Field.AsString='');
      end;
      InvalidateAllNomer;
    end;
    FRun := false;
...
begin
  ...
  if TypeRegistr=0 then begin   // вручную не указали что хотим делать
    if not DokumentDATE_OTST.IsNull then Result:=Result+8;   // дата отстранения
    if not DokumentDATE_OSV.IsNull  then Result:=Result+4;   // дата освобождения
    if not DokumentDATE_OTM.IsNull  then Result:=Result+2;   // дата прекращения
    if not DokumentUST_OTHER.AsBoolean then begin
      if not DokumentDATEZ.IsNull     then Result:=Result+1;   // дата установления
    end;
  end else begin
    Result:=TypeRegistr;
  end;
...
const
  arrFld: array[0..18] of String=('DATE_EDIT_GIS', 'DATE_POST_GIS', 'DOC_OPEKA', 'NOMER_UST','NOMER_OTM','NOMER_OTST','NOMER_OSV','DATEZ','DATE_OTM','DATE_OTST','DATE_OSV','POLE_GRN','UST_ORG','UST_ORG_SPR','UST_ORG_TYPE', 'PRICH_OTM','OTM_ORG','OTM_ORG_SPR','OTM_ORG_TYPE');
begin
