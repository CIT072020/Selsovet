...
uses
  ...
  frxCustomDB, frxAdsComponents, modify_components,adsdata//, frxBDEComponents, frxADOComponents, frxIBXComponents
{$IFDEF Delphi6}
, Variants
{$ENDIF};
...
var
  nPage:Integer;           // vadim     неправильно загружались имена страниц: Page1 и Page1  ,а должно быть Page1 и Page2
  frVersion: Byte;
  Report: TfrxReport;
  Stream: TStream;
  Page: TfrxPage;
    ...
    case LCategory of
      ...
      begin
        ...
        case LType of
          ...
          1: m.DisplayFormat.FormatStr := 'Н•в,Д†';
...
procedure ReadTfrADSQuery;      // vadim
var
  v: TfrxADSQuery;
  s: String;
begin
  v := TfrxADSQuery.Create(Page);
  TfrViewLoadFromStream;
  SetfrxComponent(v);
  v.SetBounds(-1000, -1000, 0, 0);
  ...
  frReadString(Stream);      //  DataSource
  frReadMemo(Stream, v.SQL);
  frReadBoolean(Stream);  // active
  ...
  s := frReadString(Stream);
  if s = 'CDX' then begin
    v.SourceTableType := ttAdsCDX;
  end else begin
    v.SourceTableType := ttAdsADT;
  end;
  TfrXXXDataSetReadFields;
  TfrXXXQueryReadParams(v);
  v.IsLoading := True;
...
procedure ReadTfrVCComboBoxControl;
var
 c : TfrxMComboBoxControl;
 s : String;
begin
  c := TfrxMComboBoxControl.Create(Page);
  ReadStdCtrl(c);
  c.NameOpis:= frReadString(Stream);
  c.ShortName:=frReadBoolean(Stream);
  s := frReadString(Stream);
  if c.ItemCount>0 then c.KeyIndex:=s;
end;

procedure ReadTfrVCLookupControl;
var
 l : TfrxMDBLookupComboBox;
begin
  l := TfrxMDBLookupComboBox.Create(Page);
  ReadStdCtrl(l);
  l.NameOpisEdit:= frReadString(Stream);
end;

{
procedure TfrVCLookupControl.LoadFromStream(Stream: TStream);
var
  strNameOpisEdit,strListSource : String;
  strKeyField, strListField : String;
begin
  inherited LoadFromStream(Stream);
  strNameOpisEdit := frReadString(Stream);
  strListSource   := frReadString(Stream);
  strKeyField     := frReadString(Stream);
  strListField    := frReadString(Stream);
  if strNameOpisEdit<>'' then begin
    FListSource := '';
    FNameOpisEdit := strNameOpisEdit;
    CheckOpisEdit;
  end else begin
    FListSource := strListSource;
    Prop['ListSource'] := FListSource;
    Prop['KeyField'] := strKeyField;
    Prop['ListField'] := strListField;
  end;
  Prop['BoxWidth'] := frReadInteger(Stream);
  Prop['BoxRows'] := frReadInteger(Stream);
end;
}

procedure ReadTfrVCCheckListBoxControl;
var
 c : TfrxMCheckListBoxControl;
 sl:TstringList;
begin
  c := TfrxMCheckListBoxControl.Create(Page);
  ReadStdCtrl(c);
  c.NameOpisEdit:=frReadString(Stream);
  c.ShortName:=frReadBoolean(Stream);
  c.UnionString:=frReadString(Stream);
  // DisplayField
  frReadString(Stream);
  // FieldName
  frReadString(Stream);
  ...
  //AllowGrayed
  frReadBoolean(Stream);
  c.Kav:=frReadBoolean(Stream);
end;
{
procedure TfrVCCheckListBoxControl.LoadFromStream(Stream: TStream);
var
  strNameOpisEdit : String;
begin
 inherited LoadFromStream(Stream);
 FNameOpisEdit := '';
 strNameOpisEdit := frReadString(Stream);
 FShortName := frReadBoolean(Stream);
 FUnionString := frReadString(Stream);
 FDisplayField := frReadString(Stream);
 FFieldName := frReadString(Stream);
...
begin
  ...
  begin
    ...
    begin
      ...
      ReportPage.Name := 'Page'+IntToStr(nPage);   // vadim
      Inc(nPage,1);
          ...
          gtAddIn:
            begin
              if CompareText(s, 'TfrLabelControl') = 0 then
                ...
                ReadTfrShapeView
//---------------------------------------------------------------------------------------
              else if CompareText(s, 'TfrAdsQuery') = 0 then  // vadim
                ReadTfrAdsQuery
              else if CompareText(s, 'TfrVCComboBoxControl') = 0 then  // vadim
                ReadTfrVCComboBoxControl
              else if CompareText(s, 'TfrVCLookupControl') = 0 then  // vadim
                ReadTfrVCLookupControl
              else if CompareText(s, 'TfrVCCheckListBoxControl') = 0 then  // vadim
                ReadTfrVCCheckListBoxControl
//---------------------------------------------------------------------------------------
