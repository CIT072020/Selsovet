object fmAddSobstv: TfmAddSobstv
  Left = 329
  Top = 209
  BorderStyle = bsDialog
  Caption = #1044#1086#1073#1072#1074#1080#1090#1100' '#1089#1086#1073#1089#1090#1074#1077#1085#1085#1080#1082#1072
  ClientHeight = 295
  ClientWidth = 585
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  ShowHint = True
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 8
    Top = 15
    Width = 85
    Height = 16
    Caption = #1057#1086#1073#1089#1090#1074#1077#1085#1085#1080#1082
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label2: TLabel
    Left = 8
    Top = 43
    Width = 42
    Height = 16
    Caption = #1040#1076#1088#1077#1089
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label3: TLabel
    Left = 8
    Top = 74
    Width = 88
    Height = 16
    Caption = #1063#1072#1089#1090#1100' ('#1076#1086#1083#1103')'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label4: TLabel
    Left = 8
    Top = 100
    Width = 82
    Height = 16
    Caption = #1055#1088#1080#1084#1077#1095#1072#1085#1080#1077
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label5: TLabel
    Left = 8
    Top = 194
    Width = 155
    Height = 16
    Caption = #1044#1072#1090#1072' '#1085#1072#1095#1072#1083#1072' '#1074#1083#1072#1076#1077#1085#1080#1103
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label6: TLabel
    Left = 8
    Top = 225
    Width = 177
    Height = 16
    Caption = #1044#1072#1090#1072' '#1086#1082#1086#1085#1095#1072#1085#1080#1103' '#1074#1083#1072#1076#1077#1085#1080#1103
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object edSobstv: TDBEditEh
    Left = 136
    Top = 9
    Width = 425
    Height = 26
    EditButtons = <
      item
        Glyph.Data = {
          
          186318631863}
        Hint = #1053#1072#1089#1077#1083#1077#1085#1080#1077
        Style = ebsGlyphEh
        Width = 24
        OnClick = edSobstvEditButtons0Click
      end
      item
        Glyph.Data = {
          
          276FFFFFFF44FFFFFFFFFFFF276FFFFFFFFFFFFFFFFFFFFF276F}
        Hint = #1054#1088#1075#1072#1085#1080#1079#1072#1094#1080#1103
        Style = ebsGlyphEh
        Width = 24
        OnClick = edSobstvEditButtons1Click
      end
      item
        Glyph.Data = {
          
          8888800000000888888888000008888888888888888888888888}
        Hint = #1042#1099#1073#1088#1072#1090#1100' '#1089' '#1083#1080#1094#1077#1074#1086#1075#1086' '#1089#1095#1077#1090#1072
        Style = ebsGlyphEh
        Width = 24
        OnClick = edSobstvEditButtons2Click
      end
      item
        Glyph.Data = {
          
          8888800000000888888888000008888888888888888888888888}
        Hint = #1044#1086#1073#1072#1074#1080#1090#1100' '#1089#1086#1073#1089#1090#1074#1077#1085#1085#1080#1082#1072' '#1085#1077' '#1079#1072#1088#1077#1075#1080#1089#1090#1088#1080#1088#1086#1074#1072#1085#1085#1086#1075#1086' '#1085#1072' '#1090#1077#1088#1088#1080#1090#1086#1088#1080#1080
        Style = ebsGlyphEh
        Width = 20
        OnClick = edSobstvEditButtons3Click
      end>
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    ParentShowHint = False
    ReadOnly = True
    ShowHint = True
    TabOrder = 0
    Visible = True
  end
  object edAdres: TDBEditEh
    Left = 136
    Top = 39
    Width = 425
    Height = 26
    EditButtons = <>
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 1
    Visible = True
  end
  object edTail: TDBEditEh
    Left = 136
    Top = 70
    Width = 129
    Height = 26
    EditButtons = <>
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 2
    Visible = True
  end
  object edPrim: TDBEditEh
    Left = 136
    Top = 100
    Width = 425
    Height = 81
    AutoSize = False
    EditButtons = <>
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 3
    Visible = True
    WantReturns = True
  end
  object edDateS: TDBDateTimeEditEh
    Left = 221
    Top = 190
    Width = 105
    Height = 26
    EditButtons = <>
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Tahoma'
    Font.Style = []
    Kind = dtkDateEh
    ParentFont = False
    TabOrder = 4
    Visible = True
  end
  object BitBtn1: TBitBtn
    Left = 369
    Top = 261
    Width = 90
    Height = 25
    Caption = #1047#1072#1087#1080#1089#1072#1090#1100
    ModalResult = 1
    TabOrder = 6
    Glyph.Data = {
      
      0000}
    NumGlyphs = 2
  end
  object BitBtn2: TBitBtn
    Left = 478
    Top = 261
    Width = 90
    Height = 25
    Caption = #1054#1090#1082#1072#1079
    TabOrder = 7
    Kind = bkCancel
  end
  object edDateP: TDBDateTimeEditEh
    Left = 221
    Top = 221
    Width = 105
    Height = 26
    EditButtons = <>
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Tahoma'
    Font.Style = []
    Kind = dtkDateEh
    ParentFont = False
    TabOrder = 5
    Visible = True
  end
end
