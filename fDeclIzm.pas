...
//  obj:=ObjectFromParams(slPar,'DATASET_CHNAME');
    if fmDeclIzm.GetParam('ST21','0')='1' then begin
      ...
      fmMain.ImageList.GetIcon(150,fmDeclIzm.Icon);//  btAdd.Glyph);
    end;
  end;
  try
...
begin
  if not FRunSetTypeDecl and not IsReadDokument and not IsCreateForm then begin
    ...
    try
      ...
      edTypeBrak.Enabled:=not l;

      GridDeti.Columns[6].Visible:=l;
      GridDeti.Columns[7].Visible:=l;
      GridDeti.Columns[8].Visible:=l;

      SetVisibleControls([edFamiliaNa, edNameNa, edOtchNa, lbAddSud, edAddSud, pnSud], not l);
      ...
      if FSt21 then begin
        ...
        pnBottom.Top:=pnSt21.Top+pnSt21.Height+1;
      end;
...
//  Dokument.Active:=true;
  ...
  d2xml.SetWriteFields('SPEC_D;ADD_SUD;WORK_PLACE;OTMETKA_ISP;PRIM;FIO1;FIO2;FAM1;FAM2;');
...
//    if FCheckBelName then begin
      ...
      EditDataSet(Dokument);
      Pol := tpNone;
      if DokumentST21.AsBoolean then begin
        if strField='REG_NOMER' then begin
          DokumentB_NOMER.AsString:=Field.AsString;
        end else if strField='REG_DATE' then begin
          DokumentB_DATE.AsString:=Field.AsString;
        end else if strField='REG_ZAGS' then begin
          DokumentB_ZAGS.AsString:=Field.AsString;
        end;
      end;
...
begin
  if DokumentKOD_PRICH.AsInteger>0 then begin
    edPrich.Font.Style:=[fsBold];
