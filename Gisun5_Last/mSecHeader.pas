...
//   if ActiveETSP then begin
     // если не дл€ справочника или дл€ справочника активна Ё÷ѕ
...
//      MemoWrite(ExtractFilePath(Application.ExeName)+'!!!.txt', ss);
        ...
        if (ETSP2<>nil) and ETSP2.Active then begin
  {
          if Gisun.CurAkt<>nil then begin
            ...
            Gisun.CurAkt.stBar.Panels[0].Text:='‘ормирование Ё÷ѕ ...';
          ...
          if _WorkedThread_ then begin
            ...
            try
              ...
              sOld:=getCurMessage;
              if sOld=''
                then OpenMessage('‘ормирование Ё÷ѕ ...','',0)
                else ChangeMessage('‘ормирование Ё÷ѕ ...');
            finally
              LeaveMainThread;
            end;
          end else begin
            SetOwnerForm(Gisun.CurAkt);
    //        OpenMessage('‘ормирование Ё÷ѕ ...','',0);
            sOld:=getCurMessage;
            if sOld=''
              then OpenMessage('‘ормирование Ё÷ѕ ...','',0)
              else ChangeMessage('‘ормирование Ё÷ѕ ...');
            ...
            end;
{
            ...
            Result:=false;
            s:=Avest.ErrorInfo(res);
            gisun.WriteTextLog('ошибка Ё÷ѕ: '+s,LOG_GISUN);
...
// function UTF8Encode(const WS: WideString): UTF8String;
      ...
      if sHash='' then begin
//        PutError('Ќе найдено хеш-значение, секци€ <wsse:Hash>', Gisun.CurAkt);
      end else begin
//        sHash1:=DecodeString(sHash);
        ...
        {
        if s='' then begin
          PutError('Ќе найдена секци€ <S:Body>', Gisun.CurAkt);
        end;
        }
...
//          ws:=UTF8Decode(s);
          ...
          if (sCalcHash<>'') and (sHash<>'') and (sCalcHash<>sHash) then begin
            gisun.WriteTextLog('ќшибка полученных данных: неверное ’еш-значение'+ //, Gisun.CurAkt);
                 'полученный:['+sHash+']   рассчитанный  :['+sCalcHash+']',  LOG_GISUN);
//                 'декодированный:'+sHash1+chr(13)+   // StrToHex(sHash1,' ')
            Result:=false;
          end else begin
           {
            ShowMessageCont('’еш-значение верное'+ //, Gisun.CurAkt);
