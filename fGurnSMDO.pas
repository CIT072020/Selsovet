...
type
  TfmGurnSMDO = class(TfmGurnal)
    ...
    TBItemReceiveTimer: TTBItem;
    TBSeparatorItem6: TTBSeparatorItem;
    lbReceive: TLabel;
    TBItemMailing3: TTBItem;
...
//    function LoadQuery : Boolean; override;
    ...
    procedure Event_EMLtoFTP(Sender: TObject);
    ...
    procedure Event_ViewDelMsg(Sender: TObject);
    procedure Event_CheckReadAddDokuments(Sender: TObject);
    procedure Event_CreateCopyDok(Sender: TObject);
    procedure Event_RunRefreshCOC(Sender: TObject);
    procedure Event_SetFilterAck(Sender: TObject);
    procedure Event_StatSoob(Sender: TObject);
    ...
    function ArxivEML(sFile:String; sGUID:String; nSubjType:Integer; lOpen:Boolean):String;
...
uses fMain, dBase, fChoiceNasel, fSMDODoc, fDeliverStatus;
...
begin
  ...
  inherited;
  FSimpleDel:=Globaltask.ParamAsBoolean('SMDO_SDEL'); // упрощенное удоление сообщений
...
//  cbMsgType.ItemIndex:=0;
  ...
  TBSubmenuMailing.Visible:=(Role.SystemAdmin or SMDO.IsMyOrgCIT);
...
begin
  ...
  if FEnableWrite and (SMDO.FAutoCheck>0) and (SMDO.FTypeCheckMail=TYPECHECK_RECEIVEMAIL_G) and SMDO.FPostEnabled then begin
...
//  FSeekAsQuery:=false;
  ...
  if (Role.Status=rsAdmin) or Role.SystemAdmin then begin
    ...
    it := TTbItem.Create(TBSubItemRun);
    it.Caption:='Информация об удаленных сообщениях';
    it.OnClick:=Event_ViewDelMsg;
    TBSubItemRun.Add(it);
  ...
  it := TTbItem.Create(TBSubItemRun);
  it.Caption:='Статистика входящих/исходящих';
  it.OnClick:=Event_StatSoob;
  TBSubItemRun.Add(it);
  ...
  it := TTbItem.Create(TBSubItemRun);
  it.Caption:='Обновление СОС';
  it.OnClick:=Event_RunRefreshCOC;
  TBSubItemRun.Add(it);

  {   !!!
  it := TTbItem.Create(TBSubItemRun);
  it.Caption:='Настроить фильтр уведомлений';
  it.OnClick:=Event_SetFilterAck;
  TBSubItemRun.Add(it);
  }
  ...
  if (Role.Status=rsAdmin) or Role.SystemAdmin then begin
    ...
    it := TTbItem.Create(TBSubItemRun);
    it.Caption:='Отметить доп. материалы как прочитанные';
    it.OnClick:=Event_CheckReadAddDokuments;
    TBSubItemRun.Add(it);
    ...
    it := TTbItem.Create(TBSubItemRun);
    it.Caption:='Передать сообщение разработчикам';
    it.OnClick:=Event_EMLtoFTP;
    TBSubItemRun.Add(it);
...
//-------------------------------------------------------------------------
procedure TfmGurnSMDO.Event_StatSoob(Sender: TObject);
var
  f:TfmParamQuest;
  d1,d2:TDateTime;
  n:Integer;
...
begin
  ...
  f.Caption:='Статистика входящих/исходящих';
  f.AddParamEx(Date, 'Начальная дата' , 'DATE1' ,'TYPE=DATE');
  f.AddParamEx(Date, 'Конечная дата' , 'DATE2' ,'TYPE=DATE');
  ...
  if n=1 then begin
    ...
    if dmBase.LoadSQL('СМДО_кол_док', strSQL) then begin
      strSQL:=StringReplace(strSQL, '1899-12-30', DTOS(d1,tdAds), [rfReplaceAll, rfIgnoreCase]);
      strSQL:=StringReplace(strSQL, '3000-12-30', DTOS(d2,tdAds), [rfReplaceAll, rfIgnoreCase]);
      with dmBase.WorkQuery do begin
        ...
        ss:=StringOfChar(' ',10);
        s:='  Количество зарегистрированных документов'#13#10;
        s:=s+ss+'  ВХОДЯЩИХ'#13#10;
        s:=s+ss+'    в реестре входящих: '+FieldByName('VX_ALL').AsString+#13#10;
        s:=s+ss+'    в реестре почты смдо: '+FieldByName('VX_SMDO').AsString+#13#10;
        s:=s+ss+'  ИСХОДЯЩИХ'#13#10;
        s:=s+ss+'    в реестре исходящих: '+FieldByName('ISX_ALL').AsString+#13#10;
        s:=s+ss+'    в реестре почты смдо: '+FieldByName('ISX_SMDO').AsString+#13#10;
        Close;
      end;
      ShowMessage(s);
    end else begin
      PutError('Ошибка загрузки запроса из файла');
    end
  end;
  f.Free;
end;
...
//  0:  все
    ...
    4: addWhere('smd.doc_status='+IntToStr(DOC_NOT_READ));  // непрочитан после добавления доп. материалов
...
var
  i:Integer;
  s,ss:String;
//  function CheckD:Boolean;
//  begin
//    Result:=((Query.FieldByName('SUBJ_TYPE').AsInteger=1) and (Query.FieldByName('MSG_TYPE').AsInteger<>0));  // входящий, не уведомление
//  end;
begin
  strErr:='';
  if FSimpleDel then begin
    lQuest:=true;  // спрашивать
    Result:=true;
  end else begin
    if Length(arr)=0 then begin  // удаление одной записи
      ss:='Сообщение будет удалено';
      if (Query.FieldByName('MSG_TYPE').AsInteger<>MSG_TYPE_A) and    // не уведомление
         not dmBase.DocMain.Locate('POST_ID', Query.FieldByName('ID').AsInteger, []) then begin // если во входящих не зарегистрировано, то будут удалены прикрепленные файлы
    ...
    case Key of
      ...
      VK_F2: begin
                ...
                if (Query.FieldByName('SUBJ_TYPE').AsInteger=SUBJ_OUT) then begin
                  ...
                  if n=-1 then begin
                    PutError('Сообщение не готово к отправке');
...
begin
  ...
  UpdateGbInfo;
  if Query.Active then begin
    TBItemInfoMailing.Enabled:=(Query.FieldByName('SUBJ_TYPE').AsInteger=SUBJ_OUT) and (Query.FieldByName('SENT').AsInteger=1) and
                               (ActiveControl=Grid) and
                               (Role.SystemAdmin or (Query.FieldByName('SMDOCODE').AsString=DELIVER_KEY));
  end;
end;
//-------------------------------------------------------------------------
...
begin
  ...
  if n>0 then begin
    try
      m:=StrToIntDef(Copy(ds.FieldByNAme('TEXT').AsString,1,n-1),0);
    except
    end;
  end;
...
var
  nMsgType:Integer;
begin
  nMsgType:=Query.FieldByName('MSG_TYPE').AsInteger;
  if SMDO.MsgIsAck(nMsgType) then begin
    ...
    // для входящих
    if Query.FieldByName('SUBJ_TYPE').AsInteger=SUBJ_IN then begin
      if (Query.FieldByName('DOC_STATUS').AsInteger=DOC_NOT_READ) then begin  // статус "еще не открывался"
//        AFont.Style:=[fsBold];
        Background:=clInfoBk;
      end;
      if SMDO.MsgIsAddDoc(nMsgType) then begin // если дополнение к документу и не отправлена хотя-бы квитанция о получении
        if (Query.FieldByName('ACK_TYPE').AsInteger<1)
          then AFont.Style:=[fsBold];
      end else begin                          // если основной документ и не отправлена квитанция о регистрации
        if (Query.FieldByName('ACK_TYPE').AsInteger<>2)
          then AFont.Style:=[fsBold];
      end;
    // для исходящих если не отправил
    end else if (Query.FieldByName('SUBJ_TYPE').AsInteger=SUBJ_OUT) then begin
...
begin
  ...
  if OpenDialog.Execute then begin
    ...
    SMDO.FCountAddFolder:=0;  // кол-во доп. материалов для других сообщений
    SMDO.FWriteAddFolder:=0;  // кол-во доп. материалов успешно записанных
...
//--------------------------------------------------------------
procedure TfmGurnSMDO.Event_ShowReceive(nInd:Integer; nCount:Integer);  //TReceiveMailEvent;
begin
  lbReceive.Caption:=inttostr(nInd)+'/'+inttostr(nCount);
end;
//--------------------------------------------------------------
...
//--------------------------------------------------------------
procedure TfmGurnSMDO.TBItemReceiveTimerClick(Sender: TObject);
begin
  if TBSubmenuReceive.Visible and TBSubmenuReceive.Enabled then begin
    if (FRunPost>0) then begin
...
//--------------------------------------------------------------
procedure TfmGurnSMDO.RunTimerPost;
var
  lOk,lAbort:Boolean;
  n,m:Integer;
  d:TDateTime;
...
begin
  ...
  lbReceive.Left:=GIFAnimator.Left-lbReceive.Width-3;
  lbReceive.Caption:='';
  lbReceive.Visible:=true;
  SMDO.OnUpdateWinReceiceMail:=Event_ShowReceive;
  lAbort:=false;
...
//    n:=GetTickCount-n;
      ...
      lbReceive.Visible:=false;
      lbReceive.Caption:='';
      SMDO.OnUpdateWinReceiceMail:=nil;
    end;
  except
    on EExternalAbort do begin
      lAbort:=true;
      exit; // Work was aborted by user
    end;
  end;
  if not lAbort then begin
    if n>0 then begin
      ...
      if (n=-1) and (SMDO.LastErrorSMDO<>'') then begin
...
//--------------------------------------------------------------
function TfmGurnSMDO.ArxivEML(sFile:String; sGUID:String; nSubjType:Integer; lOpen:Boolean):String;
var
  strPath:String;
begin
  Result:='';
  ...
  if FileExists(SMDO.GetPathEML+sGUID+'.eml') then begin
    ...
    Result:=Zip.FileName;
    if lOpen
      then ShellExecute(Application.Handle, PChar('explore'), PChar(strPath), nil, nil, SW_SHOWNORMAL);
  end else begin
    PutError('Файл не найден '+sGUID+'.EML');
  end;
end;
//--------------------------------------------------------------
...
begin
  ...
  if (Query.FieldByName('ID').AsString<>'') and (sGUID<>'') then begin
    ...
    ArxivEML(sFile,sGUID,Query.FieldByName('SUBJ_TYPE').AsInteger,true);
  end;
end;
//--------------------------------------------------------------
procedure TfmGurnSMDO.Event_EMLtoFTP(Sender: TObject);
var
  s,sFile,sGUID:String;
begin
  sGUID:=Query.FieldByName('MSG_ID').AsString;
  if (Query.FieldByName('ID').AsString<>'') and (sGUID<>'') then begin
    {$IFDEF POST}
    ...
    {$ELSE}
      s:='ЛАИС+';
    {$ENDIF}
    if Problem('Передать сообщение "'+Query.FieldByName('SUBJECT').AsString+'" разработчикам '+s+' ?') then  begin
      sFile:='EML_'+Query.FieldByName('ID').AsString+'.zip';
      sFile:=ArxivEML(sFile,sGUID,Query.FieldByName('SUBJ_TYPE').AsInteger,false);
      if sFile<>'' then begin
        fmMain.FileToFTP(0, sFile,false);
      end;
    end;
  end;
end;
...
//--------------------------------------------------------------
procedure TfmGurnSMDO.Event_CheckReadAddDokuments(Sender: TObject);
begin
  if Problem('Отметить все полученные доп. материалы как прочитанные ?') then begin
    dmBase.AdsConnection.Execute('UPDATE SMDOPost SET doc_status=0 WHERE doc_status='+IntToStr(DOC_NOT_READ));
    FullRefresh;
  end;
end;

//--------------------------------------------------------------
procedure TfmGurnSMDO.Event_ViewDelMsg(Sender: TObject);
var
...
//    dmBase.tbSimpleProp.SetRange([_TypeObj_IN],[_TypeObj_IN]);
  Param:=nil;
  GlobalTask.EditSpr('EDIT_DELETING', Param);
end;
    ...
    if Problem(s) then begin
      ...
      try
        with dmBase.SMDOSprOrg do begin
          if nType=3
            then Filter:='typesedrowid='+QStr(SEDVALUE_DELO)+sFilter
            else Filter:='typesedrowid='+QStr(SEDVALUE_LAIS)+sFilter;
          Filtered:=true;
          s:='';
          i:=0;
          n:=0;
          while not Eof do begin
            ...
            Inc(i,1);
            Inc(n,1);
            sList:=sList+PadLInt(n,4,' ')+'. '+FieldByName('SMDOCODE').AsString+' '+FieldByName('SHORTNAME').AsString+#13#10;
            if i>95 then begin
...
//        s:=s+#13#10+sList;
//        s:=sList;
        EditMemo_ScrollBars:=ssBoth;
        EditMemo(sList,'Список огранизаций рассылки',nil,800);
...
procedure TfmGurnSMDO.TBItemMailing3Click(Sender: TObject);
begin
  CreateMailing(3);
end;

//--- статистика рассылки ------------------------
procedure TfmGurnSMDO.TBItemInfoMailingClick(Sender: TObject);
begin
  // отправленное исходящее сообщение
  if (Query.FieldByName('SUBJ_TYPE').AsInteger=SUBJ_OUT) and (Query.FieldByName('SENT').AsInteger=1) then begin
    if Role.SystemAdmin or (Query.FieldByName('SMDOCODE').AsString=DELIVER_KEY) then begin   // рассылка
      ShowDeliverStatus(iniLocal, Query.FieldByName('ID').AsInteger);
    end;
  end;
end;

//------ настроить фильтр уведомлений  ----------
procedure TfmGurnSMDO.Event_SetFilterAck(Sender: TObject);
begin
//
end;
