...
uses
   ...
   XSBuiltIns, InvokeRegistry, uProject,
...
type
   ...
   public
      ...
      procedure PensionToLog(Log: TStrings; P: Pension; Add: string);
...
begin
   if Data<>nil then begin
      ...
      Log.Add(Format('Список ? pensions (%d):', [Length(Data.pensions)]));
      for I:=Low(Data.pensions) to High(Data.pensions) do begin
         Log.Add('');
         PensionToLog(Log, Data.pensions[I], Add+'pensions/');
      end;
   end
   else begin
      Log.Add(Add+' =nil');
...
procedure TGisunInterface.PensionToLog(Log: TStrings; P: Pension; Add: string);
begin
   if P<>nil then begin
      ...
      Log.Add(Format(Add+'suspension_date       =%s', [GetDateString(P.suspension_date)]));
      Log.Add(Format(Add+'active                =%s', [MapBool[P.active]]));
   end
   else begin                     
      Log.Add(Add+' =nil');
   end;
