...
uses
  ...
  fGurnal, Db, adsdata, adsfunc, adstable, Grids, DBGridEh, SasaDBGrid, MetaTask, dbFunc, uProjectAll,
...
type
  TfmGurnOwners = class(TfmGurnal)
    ...
    procedure edTipAdrEditButtons0Click(Sender: TObject;    var Handled: Boolean);
    procedure FormResize(Sender: TObject);
    procedure edFamiliaKeyDown(Sender: TObject; var Key: Word;   Shift: TShiftState);
    procedure GridKeyDown(Sender: TObject; var Key: Word;        Shift: TShiftState);
    procedure edTipAdrKeyDown(Sender: TObject; var Key: Word;    Shift: TShiftState);
  ...
  public
    ...
    procedure GridColumnsGetAdresOwner(Sender: TObject; EditMode: Boolean; Params: TColCellParamsEh);
...
constructor TfmGurnOwners.Create(Owner: TComponent);
begin
...
function TfmGurnOwners.LoadQuery: Boolean;
var
  i : Integer;
begin
  ...
  if Result then begin
    for i:=0 to Grid.Columns.Count-1 do begin
      ...
      if Grid.Columns[i].FieldName='ADRES_OWNER' then begin
        Grid.Columns[i].OnGetCellParams := GridColumnsGetAdresOwner;
...
procedure TfmGurnOwners.GridColumnsDom(Sender: TObject; EditMode: Boolean;  Params: TColCellParamsEh);
begin
  if Query.FieldByName('NOT_DOM').AsBoolean then Params.Text:='';
end;

procedure TfmGurnOwners.GridColumnsKorp(Sender: TObject; EditMode: Boolean; Params: TColCellParamsEh);
begin
  if Query.FieldByName('NOT_DOM').AsBoolean then Params.Text:='';
end;

//-----------------------------------------------------------
function TfmGurnOwners.getAdditiveWhere: String;
begin
//  Result:=GetStrSystemFilter(true);
  Result:='ho.date_fiks='+DateToSQL(dmBase.GetDateCurrentSost);
  if Result='' then begin
...
//-----------------------------------------------------------
function TfmGurnOwners.getDefaultAddWhere: String;
begin
  Result:=getAdditiveWhere;
//  Result:=GetStrSystemFilter(true);
end;

procedure TfmGurnOwners.GridColumnsNameDom(Sender: TObject; EditMode: Boolean; Params: TColCellParamsEh);
var
  adr:TAdres;
  s:String;
begin
...
procedure TfmGurnOwners.GridColumnsTypeKod(Sender: TObject; EditMode: Boolean; Params: TColCellParamsEh);
begin
...
procedure TfmGurnOwners.GridColumnsGetAdresOwner(Sender: TObject; EditMode: Boolean; Params: TColCellParamsEh);
var
  strName:String;
  lPropis:Boolean;
begin
...
procedure TfmGurnOwners.GridColumnsGetPassport(Sender: TObject; EditMode: Boolean; Params: TColCellParamsEh);
var
  p : TPassport;
begin
  if Query.FieldByName('TYPEKOD').IsNull or (Query.FieldByName('TYPEKOD').AsInteger=OWNER_NASEL) then begin
...
procedure TfmGurnOwners.GridColumnsGetOpisLich(Sender: TObject;
...
procedure TfmGurnOwners.SetFilter;
var
  s, strFilter : String;
  nLen : Integer;
begin
...
procedure TfmGurnOwners.edFamiliaChange(Sender: TObject);
begin SetFilter; end;

procedure TfmGurnOwners.edPunktChange(Sender: TObject);
begin
  if (edPunkt.Text = '') then begin
    QueryUL.Filter   := '';
...
procedure TfmGurnOwners.edULChange(Sender: TObject);
begin SetFilter; end;

procedure TfmGurnOwners.edTipChange(Sender: TObject);
begin SetFilter; end;

procedure TfmGurnOwners.edPunktEditButtons0Click(Sender: TObject;
          var Handled: Boolean);
begin
  Handled := true;
  edPunkt.Value:=null;
...
procedure TfmGurnOwners.edULEditButtons1Click(Sender: TObject;  var Handled: Boolean);
begin
  Handled := true;
  edUL.Value:=null;
end;

procedure TfmGurnOwners.edFamiliaEditButtons0Click(Sender: TObject;  var Handled: Boolean);
begin
  edFamilia.Text:='';
end;


procedure TfmGurnOwners.edTipEditButtons0Click(Sender: TObject; var Handled: Boolean);
begin
  edTip.ItemIndex:=-1;
end;

procedure TfmGurnOwners.edTipAdrEditButtons0Click(Sender: TObject; var Handled: Boolean);
begin
  edTipAdr.ItemIndex:=-1;
end;


procedure TfmGurnOwners.TBItemEditClick(Sender: TObject);
//var
...
procedure TfmGurnOwners.TBItemEditAdresClick(Sender: TObject);
begin
  fmMain.EditDokument(Query, dmBase.TypeObj_Adres,'ID',FListPar,true, KodGurnal);
end;

procedure TfmGurnOwners.GridDblClick(Sender: TObject);
begin
  TBItemEditClick(nil);
end;

procedure TfmGurnOwners.FormCreate(Sender: TObject);
var
  lErr : Boolean;
//  s : String;
begin
...
procedure TfmGurnOwners.FormResize(Sender: TObject);
begin
//  TBToolWindow1.DockPos:=2000;
end;

procedure TfmGurnOwners.edFamiliaKeyDown(Sender: TObject; var Key: Word;  Shift: TShiftState);
begin
  inherited;
...
procedure TfmGurnOwners.GridKeyDown(Sender: TObject; var Key: Word;  Shift: TShiftState);
begin
  inherited;
...
procedure TfmGurnOwners.edTipAdrKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  if (Key=VK_TAB) and (Shift=[]) then begin
    ActiveControl := edFamilia;
...
//--------------------------------------------------------------------
procedure TfmGurnOwners.TBItemOrder_nameClick(Sender: TObject);
begin
...
//--------------------------------------------------------------------
procedure TfmGurnOwners.TBItemOrder_adresClick(Sender: TObject);
begin
  Query.DisableControls;
  try
    Query.Close;
...
procedure TfmGurnOwners.GridGetCellParams(Sender: TObject; Column: TColumnEh; AFont: TFont; var Background: TColor;
                                     State: TGridDrawState);
begin
  if Query.FieldByName('TYPEKOD').AsInteger=1 then begin 
