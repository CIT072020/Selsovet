...
resourcestring
  ...
  SOnDiskFull ='Äèñê ïîëîí. Òğåáóåìîå ñâîáîäíîå ïğîñòğàíñòâî: %d áàéò, íî äîñòóïíî òîëüêî: %d áàéò. Î÷èñòèòå äèñê èëè íàéäèòå äğóãîé ÷èñòûé äèñê';
//  SOnDiskFull ='Disk is full. Required free space: %d bytes, but available only: %d bytes. Clean the disk or find another blank disk';
  SOnProcessFileFailure = '%s. Îøèáêà îáğàáîòêè ôàéëîâ, âîçìîæíî, äèñê çàïîëíåí';
//  SOnProcessFileFailure = '%s. File processing error, possibly disk is full';
  SWrongDiskRequestLastDisk ='File ''%s'' not found on inserted disk. Please insert last disk with required file';
...
const
  ...
  ZFErrorMessages: array[0..ZFMaxError] of string =
    ...
    'Ôàéë "%s" íå íàéäåí', //'File "%s" not found',
    'Àğõèâ íå îòêğûò', //'Archive is not open',
    'SFXStub property is not specified',
    'Íåâîçìîæíî ñîçäàòü ôàéë "%s"',   //'Cannot create file "%s"',
    'Íåâîçìîæíî ñîçäàòü ïàïêó "%s"', //'Cannot create directory "%s"',
    'Internal error. Update is not started',
    'Íåâîçìîæíî îòêğûòü ôàéë "%s"', //'Cannot open file "%s"',
    'Cannot proceed when update is not ended. Preceding EndUpdate call is required.',
    'Íåâîçìîæíî óäàëèòü ôàéë "%s"', //'Cannot delete file "%s"',
    'In-memory archive can be created only',
    'Ôàéë îòêğûò â ğåæèìå "òîëüêî äëÿ ÷òåíèÿ"', //'File is open in read-only mode',
    'Invalid compressed size, rfs.size = %d, count = %d',
    'Àğõèâíûé ôàéë çàïîğ÷åí.',  // 'Invalid archive file.',
    'Íå óäàåòñÿ ñîçäàòü âûõîäíîé ôàéë. Âåğîÿòíî, ôàéë çàáëîêèğîâàí äğóãèì ïğîöåññîì. Èìÿ ôàéëà = ''%s''.', // 'Cannot create output file. Probably file is locked by another process. FileName = ''%s''.',
    'Archive is open. You should close it beore performing this operation.',
    'Íå óäàëîñü ñîçäàòü ïàïêó', //'Unable to create directory',
    ...
    'Îøèáêà çàïèñè â ïîòîê', //'Write to stream error',
    ...
    'Íåâîçìîæíî óäàëèòü ïàïêó "%s"',  //'Cannot delete folder "%s"',
    ...
    'Íà äèñêå íåäîñòàòî÷íî ñâîáîäíîãî ìåñòà.',  //'Disk is full. There is not enough free space on disk.',
