...
uses
  ...
  ComCtrls, ExtCtrls, Grids, DBGridEh, SasaDBGrid, uCheckTreeView,
  kbmMemTable;
...
begin
  ...
  sInd:=ValueFromParams(slPar,'INDEX','LEX3_KEY');
  if sInd='NOT_RANGE' then begin
    lSetRange:=false;
  end else begin
    dmBase.AllSprGISUN.IndexName:=sInd;
...
//  old := v;
  ...
  f.EnableEdit   := false;
  SetLength(arrRec,1);
  if Trim(strSeek)<>'' then begin
    f.ActiveControl:=f.Grid;
    v:=Trim(strSeek);
  end;
  ...
  if FClass=ctZags then begin
    if Column.Field.FieldName='LEX1'then begin
      ...
      if Copy(s,1,1)='8' then begin
        ...
        end else if Copy(s,2,9)='000000000' then begin
          AFont.Style:=[fsBold,fsUnderline];
