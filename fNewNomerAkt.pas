...
begin       
  ...
  if nTypeObj=_TypeObj_DBrak then begin
    f.Caption:='Новое заявление о регистрации брака';
  end else if nTypeObj=_TypeObj_DRast then begin
    f.Caption:='Новое заявление о расторжении брака';
  end else if nTypeObj=_TypeObj_DChName then begin
    f.Caption:='Новое заявление о перемене ФИО';
  end else if nTypeObj=_TypeObj_AktZAH then begin
    f.Caption:='Новая информация';
    ...
    if lOnlySvid then begin
      f.Caption:='Повторное свидетельство записи акта';
    end;
...
//--------------------------------------------------------------------------------
procedure TfmNewNomerAkt.FormCreate(Sender: TObject);
begin
  btOk.Caption:=BT_CREATE_OK;
  btCAncel.Caption:=BT_CREATE_CANCEL;
end;
//--------------------------------------------------------------------------------
procedure TfmNewNomerAkt.UpdateActions;
begin
  inherited;
  btOk.Enabled:=( (edNomer.Value<>null) and (edDate.Value<>null) and (edOrgan.Value<>null) );
end;
//--------------------------------------------------------------------------------
procedure TfmNewNomerAkt.btOkClick(Sender: TObject);
begin
  if Check then begin
    ModalResult:=mrOk;
  end;
...
//--------------------------------------------------------------------------------
procedure TfmNewNomerAkt.FormKeyDown(Sender: TObject; var Key: Word;  Shift: TShiftState);
begin
...
//--------------------------------------------------------------------------------
procedure TfmNewNomerAkt.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
//--------------------------------------------------------------------------------
function TfmNewNomerAkt.Check: Boolean;
var
  arr:TArrStrings;
begin
  FSeekID:=0;
  if btOk.Enabled then begin
    ...
    if Result then begin
      if FTypeObj in [_TypeObj_DBrak, _TypeObj_DRast, _TypeObj_DChName] then begin
        if (edDate.Value<>null) and (VarToDateTime(edDate.Value)>Date) then begin
          Result:=false;
          PutError('Дата больше текущей');
        end;
      end;
    end;
  end else begin
