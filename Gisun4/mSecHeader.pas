...
uses  wsGisun, wsZags, wsClassif, uETSP2, uWideStrUtils, EncdDecd, StrUtils, uGisun;
...
// ----- !!! теперь создаем всегда !!! --------- 23.03.2016 для записи SignDate
         Security_.Sign := '##Sign##';
         Security_.Key  := '##Key##';
         if IsCreateHash
      ...
      if (ETSP2<>nil) and ETSP2.Active then begin
{
  ...
  //        OpenMessage('Формирование ЭЦП ...','',0);
        ...
        try
          ...
          gisun.WriteTextLog('ЭЦП прочитать "СОК"',LOG_GISUN);
          sKey:=ETSP2.GetSOK;
          gisun.WriteTextLog('ЭЦП подписать',LOG_GISUN);
          sSign:=ETSP2.Sign(Length(sUtf8),sUtf8);
          gisun.WriteTextLog('ЭЦП сформировано',LOG_GISUN);
        finally
          // работает или нет поток созданный функцией EnterWorkerThread
          if _WorkedThread_ then begin
            EnterMainThread;
