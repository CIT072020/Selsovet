...
uses
   ...
   Dialogs, DB, NativeXML, FuncPr, uWideStrUtils;
...
type
   ...
   public
      ...
      function GetPersonIdentif(const registerPersonIdentifRequest: register_person_identif_request; var registerResponse: register_response; Log: TStrings; User, Pasw: string): Boolean;
  ...
  if XMLNode<>nil then begin
    for i:=0 to XMLNode.NodeCount-1 do begin
      ...
      if s3='' then begin
        ...
        if nd<>nil then s3:=Utf8ToANSI(nd.ValueAsString);
      end;
      ...
      try
        ...
        if nd<>nil then s4:=Utf8ToANSI(nd.ValueAsString);
      except
        ////
      end;
      if s4='' then begin
      ...
      FError.AppendRecord([s1,s2,s4,s3,'','','','']);
      {
...
begin
   ...
   try
      if GetService<>nil then begin
         ...
         FCurLog:=Log;
         registerResponse:=FService.getPersonIdentif(registerPersonIdentifRequest);
         FCurLog:=nil;
...
{     необходимо исправлять
   if C<>nil then begin
      if Length(C.deathData)<>0 then begin
         for I:=Low(C.deathData) to High(C.deathData) do begin
            Log.Add(Format(Add+'deathData[%d]/death_date        =%s', [I, GetDateString(C.deathData[I].death_date)]));
            Log.Add(Format(Add+'deathData[%d]/death_date_cancel =%s', [I, GetDateString(C.deathData[I].death_date_cancel)]));
         end;
      end
      else begin
         Log.Add(Add+'deathData (0)');
      end;
      if Length(C.absentData)<>0 then begin
         for I:=Low(C.absentData) to High(C.absentData) do begin
            Log.Add(Format(Add+'absentData[%d]/absent_date        =%s', [I, GetDateString(C.absentData[I].absent_date)]));
            Log.Add(Format(Add+'absentData[%d]/absent_date_cancel =%s', [I, GetDateString(C.absentData[I].absent_date_cancel)]));
         end;
      end
      else begin
         Log.Add(Add+'absentData (0)');
      end;
      if Length(C.unefficientData)<>0 then begin
         for I:=Low(C.unefficientData) to High(C.unefficientData) do begin
            Log.Add(Format(Add+'unefficientData[%d]/unefficient_date        =%s', [I, GetDateString(C.unefficientData[I].unefficient_date)]));
            Log.Add(Format(Add+'unefficientData[%d]/unefficient_date_cancel =%s', [I, GetDateString(C.unefficientData[I].unefficient_date_cancel)]));
         end;
      end
      else begin
         Log.Add(Add+'unefficientData (0)');
      end;
      if Length(C.restrictefficientData)<>0 then begin
         for I:=Low(C.restrictefficientData) to High(C.restrictefficientData) do begin
            Log.Add(Format(Add+'restrictefficientData[%d]/restrict_unefficient_date        =%s', [I, GetDateString(C.restrictefficientData[I].restrict_unefficient_date)]));
            Log.Add(Format(Add+'restrictefficientData[%d]/restrict_unefficient_date_cancel =%s', [I, GetDateString(C.restrictefficientData[I].restrict_unefficient_date_cancel)]));
         end;
      end
      else begin
         Log.Add(Add+'restrictefficientData (0)');
      end;
   end
   else begin
      Log.Add(Add+' =nil');
