...
uses
  Windows, Messages, SysUtils, Math, Classes, WinInet, Controls, Forms, db, Dialogs, ifpii_dbfunc, dbFunc, uCheckKod, DateUtils,
  fSimpleD, uJSON, IdHTTP, IdTCPConnection, ShellApi, uTypes, kbmMemTable,
  TasksEx, AsyncCalls,
  uDataSet2XML, mRegInt, FuncPr, uProjectAll, MetaTask, OpisEdit, DBConsts, Graphics, Variants, mPermit, fLogon,
...
//    function Decode_TypePunkt(strType : String) : String;
    ...
    function CheckAllAkt(Simple: TfmSimpleD; var strError:String): Boolean;
...
uses dBase, uProject, SasaIniFile, wsZags, IniFiles, fMain, fChLoadClassif,
     ...
     {$ENDIF}
     {$IFDEF ADD_ZAGS}
       fZapisRogd, fZapisBrak, fZapisSmert, fZapisSmertV, fZapisUstOtc,
       {$IFDEF ZAGS} fRastBrak, fZapisChName, fZapisAdopt, fZapisRogdV, fZapisUstMat, {$ENDIF}
     {$ENDIF}
...
{$IFDEF MY_PROJECT}
  if (PostUserName and ((FRegInt.Username='') or (FRegInt.Password=''))) or
     ( (ActiveETSP and (not lSpr or FSprWithETSP)) and (FRegInt.PIN='')) then begin
...
begin
 ...
 //   uProject
  ...
  if FRegInt=nil
    then WriteTextLog('GISUN START >>>>>>>>>>>>>>>>>>>',LOG_GISUN);
...
//      ShowMessage(inttostr(GetLastError));
 ...
 //       showmessage(s+chr(13)+ss);
        ...
        if ETSP2.LoadLib(s) then begin
          ...
          if ETSP2.Debug then ss:=' режим отладки' else s:='';
...
//      ds.FieldByName('TYPEDATER').AsInteger:=t;
      ...
      // место рождения
      ds.FieldByName('GOSUD_R').AsString:=Decode_Alfa3(OutPut.FieldByName('K_GOSUD_R').AsString,
...
begin
  ...
  Result:=s+OutPut.FieldByName('N_TIP_UL').AsString+' '+CaseAdres(OutPut.FieldByName('N_UL').AsString);
...
begin
  ...
  Result.Ulica:=OutPut.FieldByName('N_TIP_UL').AsString+' '+CaseAdres(OutPut.FieldByName('N_UL').AsString);
  Result.Ulica_B:=OutPut.FieldByName('N_TIP_UL_B').AsString+' '+CaseAdres(OutPut.FieldByName('N_UL_B').AsString);
          ...
          if OutPut.Locate('PREFIX','ON',[]) then begin
            if (OutPut.FieldByName('K_POL').AsString<>'') and (Decode_Pol(OutPut.FieldByName('K_POL').AsString)='Ж') then begin
              ...
              if not AdresGitIsEmpty(OutPut) then begin  // если адрес места жительства не пустой !!!
                ...
                AktRogd.DokumentON_B_RESP.AsString:='';
...
N_RN_GOROD         ftString      255   ResponsePerson.data.addresses.city_region.lexema.text   ;Район города
              ...
              //место рождения
              AktRogd.DokumentON_M_B_RESP.AsString:='';
...
//              AktRogd.DokumentON_M_OBL.AsString:=Decode_Obl(OutPut.FieldByName('OBL_R').AsString,'');
        ...
        if Result and Female then begin
          if OutPut.Locate('PREFIX','ONA',[]) then begin
            if (OutPut.FieldByName('K_POL').AsString<>'') and (OutPut.FieldByName('K_POL').AsString='M') then begin
              ...
              //место жительства
              if not AdresGitIsEmpty(OutPut) then begin  // если адрес места жительства не пустой !!!
                AktRogd.DokumentONA_B_RESP.AsString:='';
  ...
  //------------------------
              ...
              //место рождения
              AktRogd.DokumentONA_M_B_RESP.AsString:='';
        ...
        if not Result then begin
          ...
          AktRogd.CheckSOATOAkt(true,true,true);
...
function TGisun.CheckAllAkt(Simple: TfmSimpleD; var strError:String): Boolean;
var
...
begin
  ...
  if Gisun.CheckCurDate then begin
    ...
    if Simple.DokumentPOVTOR.AsBoolean and not Simple.FOnlySvid then begin
      ...
      if (dDate>0) and (sNomer<>'') then begin
        if dDate>dCurDate then begin
          ...
          strError:=strError+'Дата свидетельства меньше '+DatePropis(dMinDate,3)+#13#10;
        end;
      ...
      if (Simple.TypeObj=_TypeObj_ZRast) and not Simple.FOnlySvid then begin
        if (Simple.Dokument.FieldByName('ON_SVID_DATE').AsString<>'') and (Simple.Dokument.FieldByName('ON_SVID_NOMER').AsString<>'') then begin
          ...
          if dDate>dCurDate then begin
            ...
            strError:=strError+'Дата свидетельства мужчины меньше '+DatePropis(dMinDate,3)+#13#10;
          end;
        end;
        if (Simple.Dokument.FieldByName('ONA_SVID_DATE').AsString<>'') and (Simple.Dokument.FieldByName('ONA_SVID_NOMER').AsString<>'') then begin
          ...
          if dDate>dCurDate then begin
            ...
            strError:=strError+'Дата свидетельства женщины меньше '+DatePropis(dMinDate,3)+#13#10;
          end;
        end;
      end else begin
        if (Simple.Dokument.FieldByName('DATESV').AsString<>'') and (Simple.Dokument.FieldByName('SVID_NOMER').AsString<>'') then begin
...
const
  {$IFDEF GISUN2}
    CComponentName: array [1..61] of record     // 79
  {$ELSE}
    CComponentName: array [1..59] of record     // 77
  {$ENDIF}
      Name: string;
      Code: Integer;
      Color: TColor;
     ...
     //место рождения
     (Name:'edON_M_B_RESP'; Code: bMale; Color: clDefault; nnn : 0),
...
//     (Name:'edON_RNGOROD'; Code: bMale; Color: clDefault; nnn : 0),
     ...
     //место рождения
     (Name:'edONA_M_B_RESP'; Code: bFemale; Color: clDefault; nnn : 0),
...
//    AktBrak:=TfmZapisBrak(ObjectFromParams(slPar,'FORM'));;
    ...
    if (fld<>nil) and (fld2<>nil) then begin
      ms:=Gisun.RegInt.GetMartialStatus(data, '');
...
//                AktBrak.DokumentON_REG_DATE_TILL.AsDateTime:=STOD('20180101',tdClipper);  // test
              ...
              // место рождения
              AktBrak.DokumentON_B_RESP.AsString:='';
...
//              AktBrak.DokumentON_OBL_B.AsString:=OutPut.FieldByName('OBL_B_R').AsString; //Область рождения на белорусском языке
              ...
              AktBrak.DokumentON_RAION_B.AsString:=OutPut.FieldByName('RAION_B_R').AsString; //Район рождения на белорусском языке
...
//              AktBrak.DokumentON_GOROD_B.AsString:=OutPut.FieldByName('GOROD_B_R').AsString; //Населённый пункт на белорусском языке
              ...
              // место жительства
              if not AdresGitIsEmpty(OutPut) then begin  // если адрес места жительства не пустой !!!
                AktBrak.DokumentON_M_B_RESP.AsString:='';
...
//                AktBrak.DokumentONA_REG_DATE_TILL.AsDateTime:=STOD('20171011',tdClipper);  // test
              ...
              // место рождения
              AktBrak.DokumentONA_B_RESP.AsString:='';
...
//              AktBrak.DokumentONA_OBL_B.AsString:=OutPut.FieldByName('OBL_B_R').AsString; //Область рождения на белорусском языке
              ...
              AktBrak.DokumentONA_RAION_B.AsString:=OutPut.FieldByName('RAION_B_R').AsString; //Район рождения на белорусском языке
...
//              AktBrak.DokumentONA_GOROD_B.AsString:=OutPut.FieldByName('GOROD_B_R').AsString; //Населённый пункт на белорусском языке
              ...
              //----------- место жительства ----------------
              if not AdresGitIsEmpty(OutPut) then begin  // если адрес места жительства не пустой !!!
                AktBrak.DokumentONA_M_B_RESP.AsString:='';
  ...
  //------------------------
        ...
        if not Result then begin
          ...
          AktBrak.CheckSOATOAkt(true,true,true);
...
const
  {$IFDEF GISUN2}
     CComponentName: array [1..90] of record   //
  {$ELSE}
     CComponentName: array [1..88] of record   //
  {$ENDIF}
      Name: string;
      Code: Integer;
      Color: TColor;
     ...
     (Name:'edON_B_RESP_Sv'; Code: bMale; Color: clDefault),
     ...
     (Name:'edONA_B_RESP_Sv'; Code: bFemale; Color: clDefault),
      ...
      if RequestResult=rrOk then begin
        ...
        if Person then begin
          if OutPut.Locate('PREFIX','MAN',[]) then begin
            ...
            //--------------- место рождения -----------------------
            AktSmert.DokumentRG_B_RESP.AsString:='';
            ...
            //-------------- место жительства -----------------
            if not AdresGitIsEmpty(OutPut) then begin  // если адрес места жительства не пустой !!!
              AktSmert.DokumentGT_B_RESP.AsString:='';
...
const
  {$IFDEF GISUN2}
    CComponentName: array [1..27] of record
  {$ELSE}
  ...
  {$ENDIF}
     ...
     //место рождения
     (Name:'edRG_B_RESP'; Code: bPerson; Color: clDefault),
...
const
  {$IFDEF GISUN2}
    CComponentName: array [1..64] of record
  {$ELSE}
  ...
  {$ENDIF}
     ...
     //--STATUS
     //место рождения
     (Name:'edB_RESP'; Code: bChild; Color: clDefault),
...
//            AktUstOtc.DokumentDATER.AsDateTime   := STOD( OutPut.FieldByName('DATER').AsString, tdClipper);
            ...
            //место рождения
            AktUstOtc.DokumentB_RESP.AsString:='';
        ...
        if Result and Male then begin
          if OutPut.Locate('PREFIX','ON',[]) then begin
            if (OutPut.FieldByName('K_POL').AsString<>'') and (Decode_Pol(OutPut.FieldByName('K_POL').AsString)='Ж') then begin
              ...
              //------------------место жительства ----------------------
              if not AdresGitIsEmpty(OutPut) then begin  // если адрес места жительства не пустой !!!
                AktUstOtc.DokumentON_B_RESP.AsString:='';
...
//------------------------
              ...
              //-------------------место рождения ------------------------
              AktUstOtc.DokumentON_M_B_RESP.AsString:='';
        ...
        if Result and Female then begin
          if OutPut.Locate('PREFIX','ONA',[]) then begin
            if (OutPut.FieldByName('K_POL').AsString<>'') and (OutPut.FieldByName('K_POL').AsString='M') then begin
              ...
              //------------------место жительства --------------------
              if not AdresGitIsEmpty(OutPut) then begin  // если адрес места жительства не пустой !!!
                AktUstOtc.DokumentONA_B_RESP.AsString:='';
...
//------------------------
              ...
              //------------------место рождения ------------------
              AktUstOtc.DokumentONA_M_B_RESP.AsString:='';
...
const
  {$IFDEF GISUN2}
    CComponentName: array [1..48] of record
  {$ELSE}
...
{!!!}       AktUstMat.DokumentSTATUS.AsString := '1'; //активен ??? всегда
        ...
        if Child then begin
          ...
          if OutPut.Locate('PREFIX','CHILD',[]) then begin
            ...
            //------------------место рождения -------------------------
            AktUstMat.DokumentB_RESP.AsString:='';
        ...
        if Result and Male then begin
          if OutPut.Locate('PREFIX','ON',[]) then begin
            if (OutPut.FieldByName('K_POL').AsString<>'') and (Decode_Pol(OutPut.FieldByName('K_POL').AsString)='Ж') then begin
              ...
              //-------------------место рождения ------------------------
              AktUstMat.DokumentON_M_B_RESP.AsString:='';
        ...
        if Result and Female then begin
          if OutPut.Locate('PREFIX','ONA',[]) then begin
            if (OutPut.FieldByName('K_POL').AsString<>'') and (OutPut.FieldByName('K_POL').AsString='M') then begin
              ...
              //------------------место жительства --------------------
              if not AdresGitIsEmpty(OutPut) then begin  // если адрес места жительства не пустой !!!
                AktUstMat.DokumentONA_B_RESP.AsString:='';
              ...
              //------------------место рождения ------------------
              AktUstMat.DokumentONA_M_B_RESP.AsString:='';
...
{$IFDEF ZAGS}
        ...
        if Male then begin
          if OutPut.Locate('PREFIX','ON',[]) then begin
            if (OutPut.FieldByName('K_POL').AsString<>'') and (Decode_Pol(OutPut.FieldByName('K_POL').AsString)='Ж') then begin
              ...
              //------------место рождения -----------------
              AktRastBrak.DokumentON_B_RESP.AsString:='';
              ...
              //------------ место жительства --------------------
              if not AdresGitIsEmpty(OutPut) then begin  // если адрес места жительства не пустой !!!
                AktRastBrak.DokumentON_M_B_RESP.AsString:='';
        ...
        if Result and Female then begin
          if OutPut.Locate('PREFIX','ONA',[]) then begin
            if (OutPut.FieldByName('K_POL').AsString<>'') and (OutPut.FieldByName('K_POL').AsString='M') then begin
              ...
              //-----------------место рождения -----------------------
              AktRastBrak.DokumentONA_B_RESP.AsString:='';
              ...
              //---------------- место жительства -----------------
              if not AdresGitIsEmpty(OutPut) then begin  // если адрес места жительства не пустой !!!
                AktRastBrak.DokumentONA_M_B_RESP.AsString:='';
  ...
  //------------------------
        ...
        if not Result then begin
          ...
          AktRastBrak.CheckSOATOAkt(true,true,true);
...
const
  {$IFDEF GISUN2}
     CComponentName: array [1..46] of record   //
  {$ELSE}
...
{$IFDEF ZAGS}
const
   CComponentName: array [1..77] of record   // 99       ???
      Name: string;
      Code: Integer;
      Color: TColor;
   end=(
     ...
     //--STATUS
     //место рождения
     (Name:'edB_RESP_DO'; Code: bChild; Color: clDefault),
     (Name:'edGOSUD'; Code: bChild; Color: clDefault),
     ...
     //место рождения
     (Name:'edOTEC_M_B_RESP'; Code: bMale; Color: clDefault),
     ...
     //место рождения
     (Name:'edON_M_B_RESP'; Code: bMaleU; Color: clDefault),
     ...
     //место рождения
     (Name:'edMAT_M_B_RESP'; Code: bFemale; Color: clDefault),
     ...
     //место рождения
     (Name:'edONA_M_B_RESP'; Code: bFemaleU; Color: clDefault),
...
{$IFDEF ZAGS}
const
   CComponentName: array [1..43] of record       // 29 ???
      Name: string;
      Code: Integer;
      Color: TColor;
   end=(
     ...
     (Name:'edDOK_DATE'; Code: bPerson; Color: clDefault),
     //место рождения
     (Name:'edB_RESP'; Code: bPerson; Color: clDefault),
...
//              then AktAdopt.DokumentDATER_DO.AsDateTime  := STOD( OutPut.FieldByName('DATER').AsString, tdClipper);
            ...
            //-----------------место рождения -------------
            AktAdopt.DokumentB_RESP_DO.AsString:='';
...
//                then AktAdopt.DokumentOTEC_DATER.AsDateTime := STOD( OutPut.FieldByName('DATER').AsString, tdClipper);
              ...
              //-------------место жительства
              if not AdresGitIsEmpty(OutPut) then begin  // если адрес места жительства не пустой !!!
                AktAdopt.DokumentOTEC_B_RESP.AsString:='';
              ...
              //---------------место рождения
              AktAdopt.DokumentOTEC_M_B_RESP.AsString:='';
...
//                then AktAdopt.DokumentON_DATER.AsDateTime   := STOD( OutPut.FieldByName('DATER').AsString, tdClipper);
              ...
              //-----------------------место жительства
              if not AdresGitIsEmpty(OutPut) then begin  // если адрес места жительства не пустой !!!
                AktAdopt.DokumentON_B_RESP.AsString:='';
              ...
              //------------------------место рождения
              AktAdopt.DokumentON_M_B_RESP.AsString:='';
...
//                then AktAdopt.DokumentMAT_DATER.AsDateTime   := STOD( OutPut.FieldByName('DATER').AsString, tdClipper);
              ...
              //--------------------------место жительства
              if not AdresGitIsEmpty(OutPut) then begin  // если адрес места жительства не пустой !!!
                AktAdopt.DokumentMAT_B_RESP.AsString:='';
              ...
              //--------------------------место рождения
              AktAdopt.DokumentMAT_M_B_RESP.AsString:='';
...
//                then AktAdopt.DokumentONA_DATER.AsDateTime   := STOD( OutPut.FieldByName('DATER').AsString, tdClipper);
              ...
              //------------------------------место жительства
              if not AdresGitIsEmpty(OutPut) then begin  // если адрес места жительства не пустой !!!
                AktAdopt.DokumentONA_B_RESP.AsString:='';
              ...
              //------------------------------место рождения
              AktAdopt.DokumentONA_M_B_RESP.AsString:='';
...
//            AktChName.DokumentDATER.AsDateTime   := STOD( OutPut.FieldByName('DATER').AsString, tdClipper);
            ...
            // место рождения
            AktChName.DokumentB_RESP.AsString:='';
            ...
            // место жительства
            if not AdresGitIsEmpty(OutPut) then begin  // если адрес места жительства не пустой !!!
              AktChName.DokumentM_B_RESP.AsString:='';
...
{$IFDEF ZAGS}
const
   CComponentName: array [1..54] of record      // 52  ???
      Name: string;
      Code: Integer;
      Color: TColor;
   end=(
            ...
            if Male then begin
               if OutPut.Locate('PREFIX','ON',[]) then begin
                  if (OutPut.FieldByName('K_POL').AsString<>'') and (Decode_Pol(OutPut.FieldByName('K_POL').AsString)='Ж') then begin
                     ...
                     //-------------место рождения ----------------
                     Akt.DokumentON_M_B_RESP.AsString:='';
            ...
            if Result and Female then begin
               if OutPut.Locate('PREFIX','ONA',[]) then begin
                  if (OutPut.FieldByName('K_POL').AsString<>'') and (OutPut.FieldByName('K_POL').AsString='M') then begin
                     ...
                     //место рождения
                     Akt.DokumentONA_M_B_RESP.AsString:='';
  ...
  if not SetTypeMessageAktZAH( AktSmert, strError) then begin
   ...
   //       ShowMessage(OutPut.FieldByName('DATER').AsString+'    '+Vartostr(OutPut.FieldByName('DATER').Value));
          ...
          if RegInt.Version>=4 then begin
            AktSmert.DokumentSM_B_RESP.AsString:='';
...
//              s:=s+', '+AdresToString(Output);
          ...
          AktSmert.DokumentDECL.AsString:=s;
            //----- паспортные данные (документ, удостоверяющий личность)
            {
            p := dmBase.PasportFromValues(StrToIntDef(Decode_Dokument(OutPut.FieldByName('K_DOC_TYPE').AsString),0),
                    OutPut.FieldByName('DOC_SERIA').AsString, OutPut.FieldByName('DOC_NOMER').AsString,
                    OutPut.FieldByName('N_DOC_ORGAN').AsString, '', OutPut.FieldByName('DOC_DATE').AsDateTime);
            s := dmBase.PasportToText(p,0);
            if s<>''
...
const
  CComponentName: array [1..31] of record
      Name: string;
      Code: Integer;
      Color: TColor;
   end=(
...
//     (Name:'edRG_B_OBL'; Code: bPerson; Color: clDefault),
     ...
     (Name:'edSM_B_RESP'; Code: bPerson; Color: clDefault),
...
{!!! ???}          PoleGrnSub:=PoleGrnSub or bPerson or bChildId;
          ...
          //-----------------место рождения -------------
          AktOpeka.DokumentRG_B_RESP.AsString:='';
          ...
          //----------------место жительства-----------------
          if not AdresGitIsEmpty(OutPut) then begin  // если адрес места жительства не пустой !!!
//            AktOpeka.DokumentGT_B_RESP.AsString:='';
        ...
        for i:=1 to 4 do begin
          ...
          if OutPut.Locate('PREFIX',ss,[]) then begin
            ...
            if not AdresGitIsEmpty(OutPut) and (i<>2) then begin  // если адрес места жительства не пустой !!!
              AktOpeka.Dokument.FieldByName(ss+'_B_RESP').AsString:='';
...
//!!! потом добавить    Input.FieldByName('DATE_OSV').AsString:='';
...
const
  CComponentName: array [1..14] of record
      Name: string;
      Code: Integer;
      Color: TColor;
   end=(
