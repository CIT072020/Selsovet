...
//    function AdresPropisMen(DateFiks: TDateTime; strID: String): String;
    ...
    function DeclareBrakToAkt(sl:TStringList; strSeriaSvid,strNomerSvid:String; nNomer:Integer; dDateZ:TDateTime; nLenSoato:Integer):Boolean;  // заявление о браке в а/з
...
//--------------------------------------------------------------------------------
    ...
    procedure AppendToDelObjEx(nTypeObj:Integer; strID:String; DateFiks:TDateTime; DateFiks2: TDateTime; strKomm:String);
    ...
    function InfoForDelete(ds:TDataSet; nTypeObj:Integer):String;
    ...
    procedure Event_CheckStringFilter(var sFilter:String);   // контроль значения FILTER в OpisEdit  !!!
    ...
    {
    function ATE2Record:TAte;
    function ATEfromParentAndName(nType:Integer;nID:Integer; sName:String; sKateg: String; sNotKateg: String ) : TArrAte;
    function ChoiceATEPunkt(arrATE:TArrATE):TATE;
    function GetATEAkt(ds : TDataSet; strParam: String; lCheckRnGorod:Boolean; nType:Integer) : TATE;
    function GetATEAkt2(ds : TDataSet; strParam: String; lCheckRnGorod:Boolean; nType:Integer) : TATE;
    }
    //-----------------------------
    ...
    function GetNewNomerZah(sKlad:String; sBook:String; dDate:TDateTime; nTypeNewNum:Integer):Integer;
...
//  if dDateUbit=0 then dDateUbit:=d;
  ...
  if lOk then begin
    ...
    nNewId := GetNewID( dmBase.TypeObj_Nasel );  // получим ID для старой копии человеека
...
// если nType=2  , то возвращаем '0'-част+огр  '1'-если только частные собственники  '2'-только организ.
...
{$ELSE}
 ...
 begin
   with tbOcheredResh do begin
     ...
     if ((resh.FTip=OCH_RESH_POST) or (resh.FTip=OCH_RESH_VOST)) and (Resh.FDateSILA=0) then begin // если в решении не заполнена дата постановки
...
begin
  ...
  if lOk then begin
    ...
    if not lLic and not tbMens.FieldByName('PROPIS').IsNull and (tbMens.FieldByName('PROPIS').AsBoolean=false) then begin // не прописан на територии с/с
      ID:=StrToInt(strID);
      Result.AdresPropis := Trim(dmBase.ReadOneProp(DateFiks, ID, 'ADRES_PROP', ftMemo));
      Result.Propis:=false;
      if (Result.AdresPropis='') or (Pos('не известен',ANSILowerCase(Result.AdresPropis) )>0) then begin
...
begin
  ...
  {$ELSE}
    ...
    {$ELSE}
      {$IFDEF ZAGS}
        strSQL:='SELECT MAX(id) FROM СпрМестРаботы';
  {      'SELECT MAX(id) FROM '+chr(13)+
        '  ( SELECT MAX(id) id FROM Население'+chr(13)+
        '      union all'+chr(13)+
        '    SELECT MAX(id) id FROM ЛицевыеСчета'+chr(13)+
        '      union all'+chr(13)+
        '    SELECT MAX(id) id FROM БазаДомов'+chr(13)+
        '      union all'+chr(13)+
        '    SELECT MAX(id) id FROM СпрМестРаботы'+chr(13)+
        ' ) m ';}
      {$ELSE}
        strSQL:=LoadSQLEx('MAX_ID');
      {$ENDIF}
...
function TdmBase.LgotMen2(strID:String; strDelim:String): String;  //льготы человека
begin
  Result:='';
end;
//----------------------------------------------------------
...
function TdmBase.InfoForDelete(ds:TDataSet; nTypeObj:Integer):String;
var
  s,ss:String;
begin
  if nTypeObj=_TypeObj_AktZAH  then begin
    if ds.FieldByName('NOMER').AsString=''
      then s:='б/н'
      else s:='№'+ds.FieldByName('NOMER').AsString;
    if ds.FieldByName('DATEZH').AsString<>''
      then ss:=DatePropis(ds.FieldByName('DATEZH').AsDateTime,3)
      else ss:='"пусто"';
    Result:='книга '+ds.FieldByName('BOOK').AsString+' запись '+s+' от '+DatePropis(ds.FieldByName('DATEZ').AsDateTime,3)+
      '  ИН:'+ds.FieldByName('LICH_NOMER').AsString+' дата захор. '+ss;
  end;
end;
...
begin
  ...
  Result := true;
  if nTypeObj = _TypeObj_AktZAH then begin
    sKomm:=InfoForDelete(ds, _TypeObj_AktZAH);
    d:=ds.FieldByName('DATEZ').AsDateTime;
    if AktZ.Locate('ID', sID, []) then
      AktZ.Delete;
    DeletePropSimpleDok(sID, _TypeObj_AktZAH);
    AppendToDelObjEx(_TypeObj_AktZAH, sID, d, -1, sKomm);
  end else if nTypeObj = TypeObj_Lich then begin
//    Result := DeleteLich( ds.FieldByName('DATE_FIKS').AsDateTime, ds.FieldByName('ID').AsString);
  end else if nTypeObj = TypeObj_Nasel then begin
//    Result:=DeleteMen( ds.FieldByName('DATE_FIKS').AsDateTime, ds.FieldByName('ID').AsString, true, lIgnoreCheck);
  end else if nTypeObj = TypeObj_Adres then begin
//    Result := DeleteAdres( ds.FieldByName('DATE_FIKS').AsDateTime, ds.FieldByName('ID').AsString);
  end;
end;
...
//---------------------------------------------------------------
procedure TdmBase.AppendToDelObjEx(nTypeObj:Integer; strID:String; DateFiks:TDateTime; DateFiks2: TDateTime; strKomm:String);
begin
  try
    tbDelObj.Append;
    tbDelObj.FieldByName('TYPEOBJ').AsInteger     := nTypeObj;
    ...
    tbDelObj.FieldByName('USERID').AsString       := UserId;
    if DateFiks2>0
      then tbDelObj.FieldByName('DATE_FIKS2').AsDateTime:=DateFiks2;
    if strKomm<>''
      then tbDelObj.FieldByName('KOMM').AsString:=strKomm;
    tbDelObj.Post;
    DeleteUpdating(nTypeObj, strID);
...
begin
  ...
  if s='1' then begin
    ...
    if not dmBase.IsDirBase( dmBase.GlobalPar.ConnectPath, 0 ) then begin
      lExit := false;
      while not lExit do begin
        if SelectDirectory('Выберите папку с программой на другом компьютере', '', strDir) then begin

          s0 := CheckSleshN(strDir) + 'Data\'+NAME_DICT;  // а если база не там
...
//          end;
  ...
  if Result then begin
    GlobalTask.NameFileTaskParameters:=NameFile_Params;
  end;
end;
...
//---------------------------------------------------------------------
//   контроль слова  ОБЛАСТЬ  ВОБЛАCЦЬ  для nType=1
      ...
      if Trim(arr[i])<>'' then begin
        ...
        case i of
          ...
          2 : begin
                ...
                ss := GetWordRAION(strRn,strLang); //' район';       // слово район может и не добавляться
                strRn:=Trim(strRn);
                //------------------------------
...
begin
  ...
  i:=Pos(',',sName);
  if i>0 then sName:=Trim(Copy(sName,1,i-1));
  i:=Pos('(',sName);
  if i>0 then sName:=Trim(Copy(sName,1,i-1));
...
const
  arr_type : array[0..20] of String = ('рп','г.п','гп','аг','а.г','агр','д','г. п','г','х','с','п','пос','пгт','сан','кп','нп','в/ч','в\ч','снп','в');
...
begin
  ...
  if Copy(s,1,2)='г.' then begin  // обработаем г. (город)
    if (Copy(s,3,2)<>'п.') and (Copy(s,3,3)<>' п.') then begin
      sType:='г.';
      sName:=Trim(Copy(strValue,3,length(strValue)));
      Result:=true;
      lOk:=false;
    end;
  end else if Copy(s,1,4)='дер.' then begin  //
    sType:='д.';
    sName:=Trim(Copy(strValue,5,length(strValue)));
    Result:=true;
    lOk:=false;
  end else if Copy(s,1,4)='н.п.' then begin  //
    sType:='д.';
    sName:=Trim(Copy(strValue,5,length(strValue)));
    Result:=true;
    lOk:=false;
  end else if Copy(s,1,7)='деревня' then begin  //
    sType:='д.';
    sName:=Trim(Copy(strValue,8,length(strValue)));
    Result:=true;
    lOk:=false;
  end else if Copy(s,1,11)='агрогородок' then begin  //
    sType:='агр.';
    sName:=Trim(Copy(strValue,12,length(strValue)));
    Result:=true;
    lOk:=false;
  end else if Copy(s,1,7)='поселок' then begin  //
    sType:='п.';
    sName:=Trim(Copy(strValue,8,length(strValue)));
    Result:=true;
    lOk:=false;
  end else if Copy(s,1,5)='город' then begin  //
    sType:='г.';
    sName:=Trim(Copy(strValue,6,length(strValue)));
    Result:=true;
    lOk:=false;
  end;
  if lOk then begin
    for i:=Low(arr_type) to High(arr_type) do begin
      ...
      if Result then begin
        ...
        if Copy(sType,1,3)='г.п' then
          ...
          sType:='п.'
        else if Copy(sType,1,4)='дер.' then
          sType:='д.'
...
//    nTypePunkt:=ds.FieldByName(arrF[1]).AsInteger;  // тип нас. пункт
    ...
    //------нашли несколько нас. пунктов ------------------------------------------
    if Length(arrATE)>0 then begin        // если количество нас. пунктов больше одного
      //-------- область ------------
      ...
      //--------- район ------------
//      if (sRNGorod<>'') and (sRNGorod<>'-') then lHaveRaion := true;  // район в городе
      // если название района не пусто, и это не район города
      ...
      for i:=0 to Length(arrATE)-1 do begin
        ...
        if arrATE[0].CATEGORY=112 then begin // город областного подчинения
          {$IFDEF GISUN}
  ...
  end else if (sSS<>'') then begin  // поищем по сельскому совету  без учета нас. пункта
    arrATE:=ATEfromParentAndName(tbATE, SEEK_CATEGORY, ATE_CATEG_SS, sSS,'','');
    //------нашли несколько ------------------------------------------
    if Length(arrATE)>0 then begin        // если количество больше одного
      //-------- область ------------    
      if (sObl<>'') and (sObl<>'-') then begin
        arrATE2:=ATEfromParentAndName(tbATE, SEEK_CATEGORY, ATE_CATEG_OBL, sObl,'','');
        if Length(arrATE2)>0 then idOBL:=ArrATE2[0].ATE_ID;
      ...
      //--------- район ------------
      if (sRaion<>'') and (sRaion<>'-') and (idObl>0) then begin
        arrATE2:=ATEfromParentAndName(tbATE, SEEK_PARENT, idOBL, sRaion,'','');
        if Length(arrATE2)>0
          then idRn:=ArrATE2[0].ATE_ID;
      end;
      for i:=0 to Length(arrATE)-1 do begin
        ...
        arrATE[i].idObl:=0;
        arrATE[i].idRaion:=0;
        arrATE[i].idSS:=arrATE[i].ATE_ID;
        nParent:=arrATE[i].ATE_PARENTID;
        arrATE2:=ATEfromParentAndName(tbATE, SEEK_ATEID, nParent , '', '','');  // найдем родителя
        if (Length(arrATE2)>0) and (arrATE2[0].CATEGORY=102) then begin
          nParent:=arrATE2[0].ATE_PARENTID;
          idRn2:=arrATE2[0].ATE_ID;       // район
          arrATE[i].idRaion:=idRn2;
          arrATE[i].Raion:=arrATE2[0].NAME;
          arrATE[i].Raion_B:=arrATE2[0].NAME_B;
          arrATE2:=ATEfromParentAndName(tbATE, SEEK_ATEID, nParent , '', '','');  // найдем родителя
          if (Length(arrATE2)>0) and (arrATE2[0].CATEGORY=101) then begin
            idObl2:=arrATE2[0].ATE_ID;      // область
            arrATE[i].idObl:=idObl2;
            arrATE[i].Obl:=arrATE2[0].NAME;
            arrATE[i].Obl_B:=arrATE2[0].NAME_B;
        ...
        if ( (idRn<>idRn2) or (idObl<>idObl2) or (idObl=0) ) then begin
          arrATE[i].Active:=false;
        end;
      end;
      rATE:=ChoiceATEPunkt(arrATE, lChoice);
      ...
      if Length(arrATE)>0 then begin        // если количество нас. пунктов больше одного
        ...
        for i:=0 to Length(arrATE)-1 do begin
          ...
          while (nParent>0) or (n>100) do begin
            Inc(n,1);
            arrATE2:=ATEfromParentAndName(AteSys, SEEK_ATEID, nParent , '', '','');  // найдем родителя
            if Length(arrATE2)>0 then begin
              nParent:=arrATE2[0].ATE_PARENTID;
              if arrATE2[0].CATEGORY=101 then begin
                idObl2:=arrATE2[0].ATE_ID;      // область
...
//-----------------------------------------------------------------------------------------------
function TdmBase.GetNewNomerZah(sKlad:String; sBook:String; dDate:TDateTime; nTypeNewNum:Integer):Integer;
var
  s:String;
  n:Integer;
begin
  Result:=0;
// !!! пока не проверяем блокировку if dmBase._LockTypeObj(_TypeObj_AktZah, 'База занята другим пользователем ...', true) then begin
  try
    WorkQuery.SQL.Text:='select max(nomer) from AktZ where ';
    case nTypeNewNum of
      NUM_KLAD_BOOK: s:='zh_klad='+sKlad+' and book='+sBook;   // кладбище+книга
      NUM_KLAD     : s:='zh_klad='+sKlad;                                      // кладбище
      NUM_KLAD_YEAR: s:='zh_klad='+sKlad+' and Year(datez)='+IntToStr(YearOf(dDate));   // кладбище+год
    else
      s:='zh_klad='+sKlad;   // кладбище
    end;
    WorkQuery.SQL.Text:='select max(nomer) from AktZ where '+s;
    WorkQuery.Open;
...
var
  ...
  begin
    dsLog.Append;
    dsLog.FieldByName('TEXT').AsString := strNameAkt + ' а/з №'+strNomerAkt+' от '+DatePropis(dDateAkt,0)+'г.       '+strSeria+'  №'+strNomer;
    dsLog.Post;
  end;
begin
  Result := true;
  if strNomer<>'' then begin
    if strSeria='' then begin
      ...
      if ListSvid.FieldByName('ID_ZAGS').AsInteger=Globaltask.ParamAsInteger('ID')  then begin  // только свидетельства выписанные от моего имени
...
//      lOk:=lOk and ExecuteSQL('UPDATE LoadBase.Население SET id=id+'+IntToStr(nMaxID)+', ochered=false '+strWhere);
      ...
      if lOk then begin
//        strTables:='БазаДомов,HouseOwners,SprOwners,ЛицевыеСчета,ЛицевыеСчетаПризнаки,Население,'+
        strTables:='БазаДомов,HouseOwners,HouseResh,ЛицевыеСчета,ЛицевыеСчетаПризнаки,Население,'+
...
function TdmBase.RunLoadBase(strDirSource:String; lOnlyCurSost:Boolean; lAllPunkt:Boolean; lZAGS:Boolean;
                             nNewNomerLic:Integer; strAddNomerKniga:String; slCheckSpr:TStringList) : Integer;
begin
  // для захоронений нет объединения
end;
{$ENDIF}
