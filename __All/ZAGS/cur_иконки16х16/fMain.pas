...
uses
 ...
 {$IFDEF USE_FR3} MainLibFR3, frxAdsComponents, modify_components, {frx2xto30,}
  ...
  frxExportBIFF, IdFTP, frxBarcode;
...
uses fPropertyObj, fAbout, fOperFind, fEditSpr, fChoiceZAGS2, fParamsFTP,
     ...
     fZags_SprSmert,fZags_SprRast,fZags_SprChName, fRegGISUN, fSeekAkt, fRasxodSvidNakl,fNewNomerAkt, fSimpleSeekAkt,
     fPrintSSS, fShablon, fChoiceNasel, fTableGurnal, fListSvid, fPovtorSvid, fAlfavitZAGS, DsiWin32;
...
// acQueryListIN.Visible:=IsMID(Meta);
 ...
 if lOk then begin
   ...
   with  dmBase.SprNames do begin
     ...
     if (strSOATO='') or (Copy(strSOATO,5,6)='000000') or (Copy(strSOATO,5,1)='3') or (Copy(strSOATO,5,1)='5') or (Copy(strSOATO,2,1)='4') or (Copy(strSOATO,5,1)='0')
       then FieldByName('ZAGS_SELO').AsBoolean:=false
       else FieldByName('ZAGS_SELO').AsBoolean:=true;
   end;
...
begin
  ...
  if TBItemSimpleVvod.Checked then begin
    slPar:=TStringList.Create;
...
begin
  if IsActiveGISUN then begin
    ...
    Caption:='ЗАГС [ГИС РН '+sUser+sOrg+ss+']';
...
begin
  ...
  if CanClose and (oUpdate<>nil) and oUpdate.FRun then begin
...
var
  s:String;
begin
  s:=Trim(GlobalTask.GetLastValueAsString('NOT_GISUN'));
