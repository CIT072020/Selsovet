...
begin
  dbExecuteSQL('update АктыРождений set gosud=112 where podr=9 and ((gosud=on_m_gosud and gosud<>112) or gosud is null or gosud=0)');
...
//  writedebug(sInvalidI_II);
  ...
  strSQLTrud:='select count(*) kolvo from население n '+Chr(13)+
    ...
    ' adres_id=&adres& and '+Chr(13)+
    '   ((GetVozrast(curdate(),dater)>15 and GetVozrast(curdate(),dater)<55 and pol='+QStr('Ж')+') or '+Chr(13)+
    '    (GetVozrast(curdate(),dater)>15 and GetVozrast(curdate(),dater)<60 and pol='+QStr('М')+') )  and '+Chr(13)+
    '  not exists (select kod from НаселениеЛьготы nl where nl.date_fiks=n.date_fiks and nl.id=n.id and (&kod&) )';  		  
