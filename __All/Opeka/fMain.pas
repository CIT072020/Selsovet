...
uses
  ...
  DBCtrlsEh, db, fChOrgGisun, dBase, fExportStru, fmQueryEx, MainLibFr, reportTask, uTypes,
  FR_VCCtrl, frCheckListBox, uPadegFIO, UserEvents, fParamsGisun,
  {fLichSchet, dLichSchet, fMen,dMen,} adsset, adsdata, AdsDictionary,
  fGurnal, FuncPr, FuncEh,OLEUtils, kbmMemTable, fChoiceGIS_Class,
  ...
  fSimpleD, uCheckbase, fParamQuest,ShellAPI, IdHTTP, IdComponent, fChVIgrOpeka, fChZagrOpeka,
  {$IFDEF USE_TEMPLATE}
    mTempl, mTplInt,
  {$ENDIF}
...
type
  ...
  TfmMain = class(TForm)
    ...
    acSprInternat: TAction;
    TBItemInternat: TTBItem;
    acSprUch: TAction;
    TBItemSprUch: TTBItem;
    acVigrInfo: TAction;
    acZagrInfo: TAction;
    ...
    TBItemInisSoft: TTBItem;
    TBItem5: TTBItem;
    TBItemPodr: TTBItem;
    TBItemInfoMinTrud1: TTBItem;
    TBItemInfoMinTrud2: TTBItem;
...
//    procedure SOATO_GetText(Sender: TField; var Text: String; DisplayText : Boolean);
    ...
    procedure acReOpekaExecute(Sender: TObject);
    procedure acZBrakExecute(Sender: TObject);
    procedure acZOpekaExecute(Sender: TObject);
    ...
    procedure acSprInternatExecute(Sender: TObject);
    procedure acSprUchExecute(Sender: TObject);
    procedure acVigrInfoExecute(Sender: TObject);
    procedure acZagrInfoExecute(Sender: TObject);
    procedure TBItemInisSoftClick(Sender: TObject);
    procedure acSprChildPrichExecute(Sender: TObject);
    procedure TBItemInfoMinTrud1Click(Sender: TObject);
    procedure TBItemInfoMinTrud2Click(Sender: TObject);
...
//    frxPDFExport: TfrxPDFExport;
    ...
    procedure ShowGurnal(ClassGurnal : TFormGurnal; strName : String; slPar:TStringList);
...
//    function GetPadegWord( strWord : String; Padeg : String) : String;
    ...
    procedure BeforeDeletePrichChild( Grid : TSasaDbGrid; var lDel : Boolean);
...
uses TasksEx, AsyncCalls,
     fPropertyObj, fAbout, fOperFind, fEditSpr, fParamsFTP, fChLoadClassif,
     fZapisOpeka, fGurnZOpeka, fLoad_INIS_SOFT,
...
begin
  ...
  slPar.Add('CLASS='+IntToStr(ctIspolcom));
...
begin
  sID:=Sender.AsString;
  rs:=Locate_AllSprGisun(ctIspolcom, sID);  // uProjectAll
  if rs.Ok then begin
    Text:=sID+'   '+rs.Lex1;
  end else begin
...
// acQueryListIN.Visible:=IsMID(Meta);
 ...
 if Meta.ParamAsInteger('PODR')>0 then begin
   acZagrInfo.Visible:=false;
   acVigrInfo.Visible:=true;
 end else begin
   acZagrInfo.Visible:=true;
   acVigrInfo.Visible:=false;
 end;
...
// TBIniSavePositions(Self, GlobalTask.PathService+'Menu.ini', '');
 if Role.SystemAdmin then begin
   SaveActionList(ActionList,GlobalTask.PathService+'ActionsOpeka.ini')
 end;
end;

procedure TfmMain.LoadFromIni;
begin
  LoadActionList(ActionList,GlobalTask.PathService+'ActionsOpeka.ini');
end;
...
//------------- адресная книга -------------
procedure TfmMain.acSprChildPrichExecute(Sender: TObject);
var
  Param  : TParamsEditSpr;
begin
  Param := TParamsEditSpr.Create;
  Param.OnBeforeDelete := BeforeDeletePrichChild;
  Globaltask.EditSpr('EDIT_PRICH_OPEKA', Param);
end;
procedure TfmMain.BeforeDeletePrichChild( Grid : TSasaDbGrid; var lDel : Boolean);
var
  nID : Integer;
begin
  lDel:=true;
  nID := Grid.DataSource.DataSet.FieldByName('ID').AsInteger;
  if dmBase.tbZapisOpeka.Locate('PRICH_CHILD', nID, []) then begin
    PutError(' Значение присутствует в базе.'#13#10+' Удаление невозможно.', Grid);
    lDel := false;
  end;
end;
//---------------------------------------------------------
procedure TfmMain.acSprInternatExecute(Sender: TObject);
begin Globaltask.EditSpr('EDIT_SPRINTERNAT', nil); end;
//---------------------------------------------------------
procedure TfmMain.acSprUchExecute(Sender: TObject);
...
begin
  ...
  fmParamsGisun.SetTypeBase('OPEKA');
...
//  arr:TCurrentRecord;
//  XMLDoc:TNativeXML;
//  strFaultDetail,s1,s2,s3,s4:String;
...
//  Param.OnEditRecord := EditRecordSprNames;
  Param.OnBeforeDelete := BeforeDeleteSprNames;
//  Param.OnGetCellParams := GridGetCellParams;
  Globaltask.EditSpr('EDIT_PODR_OPEKA', Param);
//    Param.Free;  удаляется после уничтожения формы
end;
//-----------------------------------------------------------------------------------------------------------------------------
...
var
  nID : Integer;
begin
  nID:=Grid.DataSource.DataSet.FieldByName('ID').AsInteger; // GlobalTask.ParamAsInteger('ID');
  if nID>0 then begin
    if dmBase.tbZapisOpeka.Locate('PODR', nID, []) then begin
      ShowMessageCont('Подразделение присутствует в базе !',Self);
      lDel := false;
    end;
  end;  
end;
  ...
  try
    if lAdd then begin
      ...
      if ds.FieldByName('HANDLED').AsBoolean then begin
        ...
        lSeek:=Gisun.SeekClassGisun(CLASS_ORGAN_REG,StrToInt(strID));
        ...
        if lSeek then begin
          f.ID := StrToInt(strID); //dmBase.SprSoato.FieldByName('KOD').AsString;
    ...
    if (f.ShowModal = mrOk) then begin
      ...
      if lAdd then begin
        ...
        if f.cbHandled.Checked then begin
          ...
          if (f.edOrgan.Text<>'') then begin //and dmBase.SprSoato.Locate('KOD',f.edSOATO.Text,[])) then begin
            nID := f.ID; // dmBase.SprSoato.FieldByName('ID').AsInteger;
          end;
        end;
      end else begin
        nID := f.ID; //dmBase.SprSoato.FieldByName('ID').AsInteger;
      ...
      if nID > -1 then begin
        ...
        if f.cbHandled.Checked then begin
          ...
          if Gisun.SeekClassGisun(CLASS_ORGAN_REG,nID) and (VarToStr(Gisun.GetValueClassGisun('SOATO'))<>'') then begin
...
procedure TfmMain.ShowGurnal(ClassGurnal : TFormGurnal; strName : String; slPar:TStringList);
var
  i : Integer;
  Gurnal : TfmGurnal;
  lCreate:Boolean;
...
//      end;
  ...
  if Gurnal<>nil then begin
    ...
    Gurnal.Show;
    SetRole;
    sPar:=ValueFromParams(slPar,'SYSFILTER','');
    if lCreate and (sPar<>'') then begin
//    OnMenu2Click
//      gurnal.FilterInterface.Filter.LoadFromFile()
//!!!    Gurnal.TBSubChoiceFlt.Items[0].Caption
      Gurnal.SetSysFilter(sPar);
    end;
  end;
  if slPar<>nil
    then FreeAndNil(slPar);
end;
...
//    TBItemSeekAndRestore.Visible:=true;
  ...
  TBItemInisSoft.Enabled    := lEnabled;
  acGisunParams.Enabled:=lEnabled;
...
procedure TfmMain.acReOpekaExecute(Sender: TObject);
begin
// реестр записей актов о смерти
  ShowGurnal(TfmGurnZOpeka, 'fmGurnZOpeka', nil);
end;

//----------------------------------
procedure TfmMain.acZOpekaExecute(Sender: TObject);
var
  nID : Integer;
  slPar:TStringList;
begin
  nID:=-1;
  slPar:=nil;
  if EditZapisOpeka(nID, slPar)
    then RefreshGurnal('fmGurnZOpeka');
end;
...
begin
  ...
  lErr := false;
  try
    dmBase.tbZapisOpeka.Exclusive:=true;
    dmBase.tbZapisOpeka.Open;
  except
    lErr := true;
    PutError(' Невозможно заблокировать базу для операции индексации ! ');
  end;
  dmBase.tbZapisOpeka.Active:=false;
  dmBase.tbZapisOpeka.Exclusive:=false;
...
begin
  ShellExecute(Application.Handle, nil, PChar(NameFromExe('\Doc\Доработки Опека и попечительство.doc')), nil, nil, SW_SHOWNORMAL);
end;
...
begin
  if IsActiveGISUN then begin
    if sUser<>'' then begin
      sUser:=' пользователь "'+sUser+'"'+'  ';
    end else begin
      if gisun.RegInt.Username=''
        then sUser:=' пользователь не установлен '
        else sUser:=' пользователь "'+gisun.RegInt.Username+'"'+'  ';
    end;      
    ...
    if Gisun.MessageSource='0'
      then sOrg:='организация не установлена '
      else sOrg:='организация "'+Gisun.MessageSource+'"  ';
    Caption:='Опека и попечительство [ГИС РН '+sUser+sOrg+ss+']';
...
begin
  if GlobalTask.ParamAsBoolean('CHECK_VOZR') then begin
    ...
    lCheck:=false;
    try
      OpenMessage('Проверка ...','');
      dmBase.WorkQuery.Open;
      if dmBase.WorkQuery.Fields[0].AsInteger>0 then begin     // ID>0
        lCheck:=true;
      end;
    except
      lErr:=true;
...
procedure TfmMain.acVigrInfoExecute(Sender: TObject);
begin
  RunVigrOpeka(0,nil);
end;

procedure TfmMain.acZagrInfoExecute(Sender: TObject);
begin
  RunZagrOpeka;
end;

procedure TfmMain.TBItemInisSoftClick(Sender: TObject);
begin
  Load_INIS_SOFT;
end;
...
procedure TfmMain.TBItemInfoMinTrud1Click(Sender: TObject);
var
  v : Variant;
  n:Integer;
begin
  if (GlobalTask.Script<>nil) then begin
    ...
    v:=null;
    GlobalTask.Script.RunEx('OpekaToMintrud',[1],v);
  end;
end;

procedure TfmMain.TBItemInfoMinTrud2Click(Sender: TObject);
var
  v : Variant;
  n:Integer;
begin
  if (GlobalTask.Script<>nil) then begin
    ...
    v:=null;
    GlobalTask.Script.RunEx('OpekaToMintrud',[2],v);
  end;
end;
