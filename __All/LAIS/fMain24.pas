...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, dbt, EncdDecd, VistaAltFixUnit,
  MetaTask, mPermit, frRtfExp, frOLEExl, FR_Class, FR_E_HTML2, FR_DCtrl, FR_AdsDb, uDataSet2XML,
...
//    function  ChoiceFileFTP:String;  // FTP
   //---------------------------------------------------
    ...
    function SMDOReceive(lStart:Boolean; lVisible:Boolean; lCheck:Boolean; lOne:Boolean):Integer;
...
uses fPropertyObj, fAbout, fOperFind, fEditSpr, fChoiceZAGS,
     ...
     NewDialogs, uFilter, fmDosFileView, uProjectAll,
     fZapisBrak, fZapisRogd, fGurnZRogd, fZapisSmert, fGurnZSmert, fChVigrOch,
     {fZapisBrakV, fZapisRogdV, fZapisSmertV,}
     fZapisUstOtc, fGurnZUstOtc, fLoadFromBTI,{ fLoadIN_Mens,} fEditSprWork,
     {$IFDEF ADD_ZAH}
       fAktZ, fGurnAktZ, fEditRecSprZAH,
     {$ENDIF}
     {$IFDEF ADD_OPEKA}
       fZapisOpeka, fGurnZOpeka,
     {$ENDIF}
     fDeclRegistr, fParamsFTP,
...
procedure TfmMain.SOATO_GetText(Sender: TField; var Text: String;
           DisplayText : Boolean);
var
  strType, strSOATO, strSS, strRN, strOBL, sID : String;
begin
...
begin
  CheckUpdate(IdFTP1, true, true, 0, true, strFileUpdate);
{
  FRunAddMenu:=true;
  SetRole;
  try
  ...
  if (strErr='') and (s<>'') then begin
    if Problem('Доступно обновление '+Gisun.NameUpdate+'. Загрузить?') then begin
      if Gisun.DowloadUpdateProg(s, strErr) then begin
...
//  ShowMessage(VarToStr(v));
  ...
  {
  TypeServerFTP:=0;
  SetPropertyFPT('update');
  GetFileFTP('');
  for n:=0 to idFTP1.DirectoryListing.Count-1 do begin
    Memo1.Lines.Add(idFTP1.DirectoryListing.Items[n].FileName);
  end;
  }
  {
  TypeServerFTP:=0;
...
//  ShowMemo(slPar,'test', '0;0;600;200');
//  FreeAndNil(slPar);
...
//  ListParamsSpr.AddObject(Opis.Caption, Param);
  ...
  {$ENDIF}
  Param.FreeOnExitSpr:=false;
  Opis:=GlobalTask.CurrentOpisEdit.GetSprOpisA('EDIT_SPRWORK');      Opis.ParamsEditSpr:=Param;
  Opis:=GlobalTask.CurrentOpisEdit.GetSprOpisA('EDIT_WORK_ZAGS');    Opis.ParamsEditSpr:=Param;
  Opis:=GlobalTask.CurrentOpisEdit.GetSprOpisA('LOOKUP_WORK');       Opis.ParamsEditSpr:=Param;
  Opis:=GlobalTask.CurrentOpisEdit.GetSprOpisA('LOOKUP_WORK_ZAGS');  Opis.ParamsEditSpr:=Param;
  Opis:=GlobalTask.CurrentOpisEdit.GetSprOpisA('LOOKUP_WORK_NAME');  Opis.ParamsEditSpr:=Param;
  ListParamsSpr.AddObject(Opis.Caption, Param);
  //---------------------------------------------------------------------
    ...
    if dmBase.LastDatabaseError=0 then begin
      ...
      {$IFDEF SMDO}
        if SMDOEnabled and acSMDOPost.Visible and  acSMDOPost.Enabled then begin
          SMDOReceive(true,true,true,false);
        end;
      {$ENDIF}
    end;
...
begin
  if IsActiveGISUN then begin
    ...
    if Gisun.ActiveETSP then ss:='(ЭЦП)' else ss:='';
...
//    if not Gisun.DbUserAsGIS then begin
      ...
      ss:='';
      sPIN:='';
      if Gisun.ActiveETSP then n:=2 else n:=1;
...
//----------------------------------------------------------------
//
function TfmMain.SMDOReceive(lStart:Boolean; lVisible:Boolean; lCheck:Boolean; lOne:Boolean):Integer;
{$IFDEF SMDO}
var
  n:DWORD;
  d:TDateTime;
...
{$IFDEF SMDO}       
  ...
  if lStart then begin
    ...
    if SMDOEnabled and Avest.IsActive then begin
      n:=0;
      sErr:='';
      if SMDO.FRefreshCOCReceive and SMDO.FCheckStart then begin // обновлять СОС перед получением почты
      ...
      if n=3895066949 then begin
        //  В справочнике списков отозванных сертификатов имеется более новый (такой же) СОС данного издателя.
  ...
  if SMDOFullEnabled then begin
    if not lStart or SMDO.FCheckStart then begin
      ...
      if m>0 then begin
        if not lRefreshCOC and SMDO.FRefreshCOCReceive then begin // обновлять СОС перед получением почты
        ...
        lOk:=SMDO.ReceiveMail(slPar,nCount,nCountErr,nCountUvd,lOne);
        Result:=nCount;
        if lVisible then begin
          if lOk then begin
            if nCount>0 then begin
              ...
              ShowMessage('Нет новых сообщений');
            end;
          end else begin
            SMDO.ShowError;
          end;
