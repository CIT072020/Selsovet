...
type
  ...
  public
    ...
    procedure AddFile(sFileName:String; nPostID:Integer; nParentID:Integer; sSign:String);
    procedure EditKomu;
    ...
    function ChoiceFromSMDO(nType:Integer; var arrRec:TCurrentRecord):Integer;
...
uses dBase, uSMDO, fMain, fEditSign, fEditSpec, mDRecInt, fEditSpr, funcEh, SelLibFR, fShablon,
  StrUtils;
...
//    nDocStatus := f.DokumentDOC_STATUS.AsInteger;
...
begin
  ...
  //      DokumentDOC_STATUS.AsInteger:=DOC_CREATE;
  //      if SubjIN
  //        then DokumentDOC_STATUS.AsInteger:=DOC_REGISTER;
      end;
    end;
    f.Free;
  end;
...
var
  s,sRegNumber,sIDNumber, sBMPost, sBMTask:String;
  dDate:TDateTime;
  nID,nType:Integer;
  tbList:TDataSet;
  lErr:Boolean;
begin
  ...
  nType:=GlobalTask.ParamAsInteger('FOLDER_UPDATE');
  ...
  if (DokumentISWRITE_REF.AsBoolean=false) and (DokumentSMDOCODE.AsString<>'') and SubjIN and (tbReferred.RecordCount>0) and (nType<>3) then begin
    lErr:=false;
    tbList:=dbCreateMemTable('PARENT_ID,Integer;IDNUMBER,Char,36;REGNUMBER,Char,30;REGDATE,Date,0;RETYPE,Integer','');
    tbList.Name:='tbList';
    tbList.Open;
...
var
  n,nDocID,nFolderID, nCurFolderID, nCurID :Integer;
...
//      FMainTable.FieldByName('DOC_STATUS').AsInteger:=DokumentDOC_STATUS.AsInteger;
...
//------------------------------------------------------------
procedure TfmSMDODoc.AddFile(sFileName:String; nPostID:Integer; nParentID:Integer; sSign:String);
var
  sSignInfo, s,sFile,sPath,sFileReal,sPathReal:String;
  lCopy:Boolean;
...
var
  ...
  v : Variant;
  arrRec : TCurrentRecord;
  nPostID, nPostOtvNa, i, n, nDocID:Integer;
  ds:TAdsTable;
  lOk,lNotVid:Boolean;
  Enum: TEnumItem;
...
begin
  ...
  //    dmBase.LookUpDocMain.SQL.Text    если понадобиться изменить запрос
    ...
    if Opis <> nil then begin
        ...
        if Opis.ChoiceFromSprEx(nil, v, arrRec, nil) and (v<>null) then begin
          if v <> null then begin
            ...
            if sOrg<>'' then begin
              ...
              if SMDO.OrgIsOneWindow(sOrg) // одно окно
                then DokumentPAR_DELIVERY_TYPE.AsInteger:=DELIVERY_TYPE_SMDO;
                  ...
                  while not Eof do begin
                    ...
                    if sFileName<>'' then begin
                      if FileExists(sFileName) then begin
                        AddFile(sFileName, nPostID, nPostID, '');
                      end else begin
      ...
      if lNotVid then ShowMessage(PadCStr('Выберите вид документа',40,' '));
    end;
  end;
end;
...
begin
  ...
  if getDeliveryType=DELIVERY_TYPE_SMDO then  begin
    ...
    if Opis <> nil then begin
      v := null;
      dmBase.SMDOPost.Filter:='SUBJ_TYPE=1 and MSG_TYPE<>0';   // входящее сообщение, не уведомление
...
//---------------------------------------------------------------------
function TfmSMDODoc.ChoiceFromSMDO(nType:Integer; var arrRec:TCurrentRecord):Integer;
var
  Opis : TOpisEdit;
  v : Variant;
//  arrRec : TCurrentRecord;
...
begin
  ...
  if Opis <> nil then begin
    ...
    dmBase.SMDOPost.Filter:='SUBJ_TYPE='+IntToStr(nType)+' and (MSG_TYPE=1 or MSG_TYPE=3) and SUBJ_ERROR=false';   // входящее или исходящее сообщение, не дополнение, не уведомление, не ошибочное
    dmBase.SMDOPost.Filtered:=true;
    try
      if Opis.ChoiceFromSprEx(nil, v, arrRec, nil) and (v<>null) then begin
        Result:=v;
...
//  ShowMessage(ActiveControl.Name);
  ...
  tbReferred.First;
  if n>0 then begin
    ShowMessage('Допускается только одна ссылка');
    exit;
  end;
  nID:=ChoiceFromSMDO(2, arrRec);
  if nID>0 then begin
  ...
  if SelectDirectory('Выберите папку', '', sDir) then begin
    ...
    if FileExists(sFile) then begin
      ...
      if lBase64 then begin
        ...
        s:=StringReplace(ds.FieldByName('SIGNATURE').AsString, chr(13)+chr(10) ,'' , [rfReplaceAll]);
        MemoWrite(sDir+'эцп'+IntToStr(i+1)+'.p7s', DecodeString(s));  // ??? DecodeString(slSign.Strings[i])
      end;
      if CopyFile(PChar(sFile), PChar(sFileC), true) then begin
        ShellExecute(Application.Handle, PChar('explore'), PChar(sDir), nil, nil, SW_SHOWNORMAL);
//        ShellExecute(Application.Handle, nil, PChar(sFileC), nil, nil, SW_SHOWNORMAL);
