...
type
  ...
  public
    ...
    function RefToSMDO(nodeOwner:TXMLNode; nIDRef:Integer; sSmdoCode:String; var sErr:String):Boolean;
...
//    function ReceiveMailTest:Boolean;
    ...
    function CheckReceiveMail(lShow:Boolean):Integer;
    ...
    function CheckElFolder(node:TXMLNode; nFolder:Integer; sSmdoCode:String):Boolean;
...
//  Avest.OnWriteLog:=
  ...
  if sPath<>'0' then begin
    ...
    if FEnabled and FLocalEnabled then begin
      ...
      {
      if not Avest.IsActive or (sErr<>'') then begin
        if sErr=''  then sErr:='неизвестная ошибка';
        if sPath='' then sPath:=NAME_AVEST_DLL; //'AvCryptMail.dll';
        sErr:='Ошибка загрузки библиотеки ЭЦП Avest ('+sPath+') : '+sErr;
        WriteTextLog(sErr);
      end else begin
//        WriteTextLog('Библиотека ЭЦП AVEST загружкна');
      end;
      }
    end else begin
...
function TSMDO.CheckReceiveMail(lShow:Boolean):Integer;
var
  idPOP:TidPOP3;
  Msg:TidMessage;
  IdSocksInfo:TIdSocksInfo;
...
begin
  ...
  if FBeforeCheckTCP then begin
    if not CheckTCP(FPopServer, FPopPort, lShow) then begin
      Result:=-1;
      exit;
    end;
  end;
...
var
  s,s1,s2,s3,s4,sSysID,sSystem,sSystemDetails,sMsgID,sSubject,sNode,sOrgID,sOrgName:String;
  ...
  nCountPost, nPostID,nTaskID,nMsgType,i,j,n,nMsgAckNow, nAckType, nErrDoc:Integer;
...
//  sFrom:=sFrom;    // адрес отправителя
  ...
  nMsgType:=StrToIntDef(s,-1);
  if (nMsgType<0) or (nMsgType>4) then AckResultAdd(ceVID,'','','');
...
//        end;
  ...
  if FAckResult.Count>0 then begin
    ...
    if node_receiver<>nil then begin
      if not RefToSMDO(node_receiver, 0, sOrgID, sErr) then begin // разберем из Receiver его Referred
        AckResultXML(sErr);
      end;
    end;
    ...
    if nMsgType=MSG_TYPE_A then begin
      ...
      if FAckResult.Count=0 then begin
        with dmBase.SMDOPost do begin
          ...
          FieldByName('MSG_TYPE').AsInteger:=nMsgType;
...
//                FieldByName('DOC_STATUS').AsInteger:=DOC_REGISTER;      // получена квитанция о рег. документа
    ...
    //----------------------------------------------
      ...
      if node=nil then begin
        WriteTextLog('Получено сообщение ('+sType+') : '+NameMsgType(nMsgType));
        lOk:=true;
        ...
        if not MsgIsAddDoc(nMsgType) and (node_doc=nil) then begin
        ...
        if (nMsgType=MSG_TYPE_DA) and (ListTask.Count=0) and (ListFolder.Count=0) then begin
        ...
        end else if (nMsgType=MSG_TYPE_OA) and (ListFolder.Count=0) then begin
    ...
    //    BeginTransaction;
          ...
          if lOk and (ListTask.Count>0) then begin
            for i:=0 to ListTask.Count-1 do begin
              lOk:=CheckElTask(TXMLNode(ListTask[i]), i+1, dmBase.SMDOPost.FieldByName('IDNUMBER').AsString, sOrgID );
            end;
          end;
          if lOk and (ListFolder.Count>0) then begin
            for i:=0 to ListFolder.Count-1 do begin
              lOk:=CheckElFolder(TXMLNode(ListFolder[i]), i+1, sOrgID);
            end;
          end;
          if MsgIsAddDoc(nMsgType) then begin
          ...
          //          if lOk then begin                  !!! !!!
            with dmBase.SMDOPost do begin
              ...
              FieldByName('SUBJECT').AsString:=sSubject;
              FieldByName('SUBJ_SYSTEM').AsInteger:=getSubjSystem(sOrgID);
              if MsgIsAddDoc(nMsgType) and (FieldByName('ADR_REGNUMBER').AsString='') then begin
  ...
  //            FieldByName('DOC_STATUS').AsInteger:=DOC_PRESENT; // CREATE;
              ...
              //=======================================
              if lOk then begin
                CreateAckResult(DOC_CREATE, RESULT, false, nMsgType)
              end else begin
                CreateAckResult(DOC_ERROR, RESULT, false, nMsgType);
              end;
            end;
            ...
            FDocTask.First;
            if not FDocTask.Eof  then begin
            sl.Clear;
            FMens.Filter:='';
            ...
            for i:=0 to sl.Count-1 do begin
              ...
              s:=sl.Names[i];                           // псевдо ID
              FReferred.Filter:='parent_id='+s;   // !!!
            ...
            for i:=0 to sl.Count-1 do begin
              nTaskID:=StrToInt(sl.ValueFromIndex[i]);  // реальный ID  или 0
              s:=sl.Names[i];                           // псевдо ID
              FFiles.Filter:='parent_id='+s;   // !!!
              FFiles.Filtered:=true;
              FMens.Filter:='parent_id='+s;   // !!!
              FMens.Filtered:=true;
              n:=1;
              while not FMens.Eof do begin
  ...
  //              ShowMessage(inttostr(dmBase.SMDOFile.FieldCount)+chr(13)+Inttostr(FFiles.FieldCount) );
...
//          end;     lOk               !!! !!!
  ...
  //============================
  if (nMsgType<0) or (nMsgType>4) then nMsgType:=1;
...
var
  sErr,s,sNode,sNameFile,sIdNumber,sNameRef:String;
  node1,node2:TXMLNode;
  d:TDateTime;
  List:TList;
...
begin
  ...
  sErr:='';
  if not RefToSMDO(node, nTask, sSmdoCode, sErr)
...
//-----------------------------------------------------------------------
function TSMDO.CheckElFolder(node:TXMLNode; nFolder:Integer; sSmdoCode:String):Boolean;
var
  sErr,s,sNameFile,sNameRef:String;
  node1,node2:TXMLNode;
  d:TDateTime;
  List:TList;
  i,n,j:Integer;
...
begin
  ...
  n:=FAckResult.Count;  // количество записей в списке ошибок
  FDocTask.Append;
  FDocTask.FieldByName('task_id').AsInteger:=nFolder;   // в базе это autoinc
  ...
  sErr:='';
  if not RefToSMDO(node, nFolder, sSmdoCode, sErr)
    then AckResultXML(sErr);
...
begin
  if nType=0 then begin
    ...
    Result:='(smd.subj_type=2 and (smd.sent=0 or (smd.msg_type>0 and not smd.subj_error and smd.ack_type<>'+IntToStr(ACK_REGISTER)+' )) ) or '+
            '(smd.subj_type=1 and smd.msg_type>0 and not smd.subj_error and smd.subj_system=0 and smd.ack_type<>'+IntToStr(ACK_REGISTER)+')';
    ...
    Result:=s+'smd.subj_type=1 and smd.msg_type>0 and not smd.subj_error and smd.subj_system=0 and smd.ack_type<>'+IntToStr(ACK_REGISTER); // входящие докум. незарег. (без уведомления)
  end;
...
//------------------------------------------------------
function TSMDO.RefToSMDO(nodeOwner:TXMLNode; nIDRef:Integer; sSmdoCode:String; var sErr:String):Boolean;
var
  i,nRetype:Integer;
  node:TXMLNode;
  sIdNumber,sAtrDate,sRegNumber:String;
  ...
  for i:=0 to nodeOwner.NodeCount-1 do begin
    if ANSILowerCase(nodeOwner.Nodes[i].Name)='referred' then begin
      ...
      if (node<>nil) and (node.ValueAsString<>'') then begin
        ...
        FReferred.Post;
      end else if sIdNumber<>'' then begin
...
// f.cbDeletePost.Checked:=FDeletingPost;
 ...
 if FVersionXML_OUT=100
   then f.cbVersion.ItemIndex:=0
   else f.cbVersion.ItemIndex:=1;
...
//   Ini.WriteBool('POST', 'AUTHORIZATION', FAutho);
   ...
   Ini.WriteString('POST', 'USER  NAME', FUserName);
