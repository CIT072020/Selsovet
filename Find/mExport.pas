unit mExport;
//todo99: не работает - memo в dbf
//todo98: добавить выгрузку таблицы по шаблону (rtf, xls)
interface
uses
...
const
   DefBooleanName: array [Boolean] of string=('Нет', 'Да');
...
type
   ...
   //todo7: ? перенести под условную компиляцию TExportOracleQuery и TExportOracleDataSet из egr
...
procedure DoExportData(FileName, Msg, ExcludeFields: string; Data: TObject; ExportColumns: TExportColumnList; Target: TExportFormat; ShowResult, Hide, ShowGauge: Boolean; Interval, ReadBuffer: Integer; lLandscape:Boolean; slBookmark:TStringList);
//++
...
begin
   ...
   try
      if Data is TDataSet then begin
         ...
         if (slBookmark<>nil) and (slBookmark.Count>0) then begin
           TExportDataSet(ExportData).FBookmark.Assign(slBookmark);
         end;  
      end
      ...
      if ExportData<>nil then begin
         ExportData.Landscape:=lLandscape;
         if ExportColumns<>nil then begin
...
procedure TExportColumnList.Delete(I:Integer);
var
   Item: TExportColumn;
begin
...
begin
   if FBookmark.Count=0 then begin
     Result:=FDataSet.Eof;
   end else begin
     if FCount>FBookmark.Count
       then Result:=true
       else Result:=false
   end;
end;
...
begin
   ...
   if FBookmark.Count=0 then begin
     ...
     FDataSet.Bookmark:=FBookmark.Strings[FCount-1];
   end;
end;
...
begin
   ...
   if FBookmark.Count=0 then begin
     ...
     if FCount<=FBookmark.Count then begin
       FDataSet.Bookmark:=FBookmark.Strings[FCount-1];
     end else begin
       FDataSet.Bookmark:=FBookmark.Strings[FBookmark.Count-1];
     end;
   end;
end;
...
begin
   ...
   if I<0 then begin
      ...
      case Field.DataType of
         ftString, ftWideString, ftMemo, ftFMTBcd, ftBcd: begin
         ...
         ftTime : begin
            if not Field.IsNull then begin
               Result:=FormatDateTime(FTimeFormat, Field.AsDateTime);
            end;
         end;
//         ftDate, ftDateTime: begin    VADIM   06.04.2020
//            if not Field.IsNull then begin
//               Result:=FormatDateTime(FDateFormat, Field.AsDateTime);
//            end;
//         end;
         ftDate: begin              //  VADIM   06.04.2020
            if not Field.IsNull then begin
               Result:=FormatDateTime(FDateFormat, Field.AsDateTime);
            end;
         end;
         ftDateTime: begin         //  VADIM   06.04.2020
            if not Field.IsNull then begin
               Result:=FormatDateTime('dd.mm.yyyy hh:nn:ss', Field.AsDateTime);
               if Copy(Result,12,8)='00:00:00'
                 then Result:=Copy(Result,1,10)
            end;
         end;
         ftBoolean: begin
            {--
...
begin
   ...
   for I:=0 to Pred(FieldCount) do begin
      ...
      //todo8: сделать возможность замены имен полей для dbf см. lpx или сделать параметром поля
...
var
   ...
   TempBooleanName: array [Boolean] of string;
begin
   Result:=False;
   ...
   DbfFile:=TDBFFile.Create(FFileName);
   try
      TempBooleanName[False]:=DefBooleanName[False];
      TempBooleanName[True]:=DefBooleanName[True];
      ...
      DefBooleanName[False]:=TempBooleanName[False];
      DefBooleanName[True]:=TempBooleanName[True];
...
begin
   ...
   try
      ...
      Writeln(RtfFile, '{\rtf1\ansi\deff0\deflang1049\deflangfe1049');
      PrintFontTable;
      PrintColorTable;
      if FExportData.Landscape
        then Writeln(RtfFile, '\paperw16838\paperh11906\margl1134\margr567\margt1134\margb1134\landscape') //A4 альбомная
        else Writeln(RtfFile, '\paperw11906\paperh16838\margl1701\margr850\margt1134\margb1134'); //  vadim
      ...
      for Col:=0 to Pred(FieldCount) do begin
         ...
         if ExportColumns[Col].BgColorIndex=1{clWhite} then begin
            if ExportColumns.GridLine then begin
               CellHead[Col]:=Format('\clbrdrt\brdrs\clbrdrl\brdrs\clbrdrb\brdrs\clbrdrr\brdrs\cellx%d', [cellx[Col]]);
            end
            else begin
               CellHead[Col]:=Format('\cellx%d', [cellx[Col]]);
            end;
            ...
            if ExportColumns.GridLine then begin
               CellHead[Col]:=Format('\clbrdrt\brdrs\clbrdrl\brdrs\clbrdrb\brdrs\clbrdrr\brdrs\clcbpat%d\cellx%d', [ExportColumns[Col].BgColorIndex, cellx[Col]]);
            end
            else begin
               CellHead[Col]:=Format('\clcbpat%d\cellx%d', [ExportColumns[Col].BgColorIndex, cellx[Col]]);
            end;
         end;
         CellData[Col]:=Format('\pard\intbl\%s\sb55\sa15\f0\fs%d\cf%d ', [MapAlignment[ExportColumns[Col].Alignment], FontSize, ExportColumns[Col].FgColorIndex]);
      end;
      if cellx[Pred(FieldCount)]>CMaxTableWidth then begin
         Koef:=(CMaxTableWidth-FieldCount*(2*trgaph))/(cellx[Pred(FieldCount)]-FieldCount*(2*trgaph));
         for Col:=0 to Pred(FieldCount) do begin
...
var
   ...
   //todo2: вынести ParseHead в методы, что бы можно было добавить свой код см. BelarusInfo
   ...
   begin
      ...
      for Col:=0 to Pred(FieldCount) do begin
         ...
         repeat
            X:=Pos('|', TempStr);
            if X<>0 then begin
               HeadCell[Col][Row].S:=Trim(Copy(TempStr, 1, Pred(X))); //todo1: trim head параметром см. BelarusInfo
               Inc(Row);
               SetLength(HeadCell[Col], Succ(Row));
               TempStr:=Trim(Copy(TempStr, Succ(X), Length(TempStr)-X)); //todo1: trim head параметром см. BelarusInfo
               HeadCell[Col][Row].S:=TempStr;
            end;
         until X=0;
         if MaxRow<Row then begin
   ...
   //todo3: возможность задвать параметры таблицы шрифтов см. BelarusInfo
   //todo4: возможность добавления своих стилей см. BelarusInfo
   ...
   begin
      ...
      for I:=0 to Pred(FDataProviderList.Count) do begin
         ...
         XF.B1:=haLeft;
         Workbook.Stream.Write(XF, SizeOf(TXFRec));
         // стиль "влево" и "перенос по словам"
         Inc(LastXF);
         //LeftXF[I]+1
         XF.B1:=haLeft or fWrap;
         Workbook.Stream.Write(XF, SizeOf(TXFRec));
         // стиль "вправо"
         Inc(LastXF);
         RightXF[I]:=LastXF;
         XF.B1:=haRight;
         Workbook.Stream.Write(XF, SizeOf(TXFRec));
         // стиль "вправо" и "перенос по словам"
         Inc(LastXF);
         //RightXF[I]+1
         XF.B1:=haRight or fWrap;
         Workbook.Stream.Write(XF, SizeOf(TXFRec));
         // стиль "середина"
         Inc(LastXF);
         CenterXF[I]:=LastXF;
         XF.B1:=haCentred;
         Workbook.Stream.Write(XF, SizeOf(TXFRec));
         // стиль "середина" и "перенос по словам"
         Inc(LastXF);
         //CenterXF[I]+1
         XF.B1:=haCentred or fWrap;
         Workbook.Stream.Write(XF, SizeOf(TXFRec));
      end;
   ...
   function GetSST(S: string; var WrapStyle: Integer): Integer;
   var
      Ind: Integer;
   begin
      Inc(NumSST);
      //для многострочных полей необходимо присвоит другой стиль и убрать лишьний символ
      WrapStyle:=0;
   ...
   //todo5: возможность указывать стили для любой ячейки шапки см. BelarusInfo
   procedure PrintHead;
   var
      I: Integer;
      R: Integer;
         ...
         for J:=0 to Pred(FDataProviderList.Count) do begin
            ...
            try
               ...
               while not FExportData.GetEof and (RowCount<CMaxExcelRow) do begin
                  ...
                  for I:=0 to Pred(FieldCount) do begin
                     FieldNo:=ExportColumns[I].FieldIndex;
                     //todo9: вынести конкретную проверку в отдельную процедуру что бы можно было бы её исключить или сделать параметром поля
                     if ((Pos('POKAZ_', FExportData.GetFieldName(FieldNo))=1) or
                        (Pos('COUNT_', FExportData.GetFieldName(FieldNo))=1)) and
                        FExportData.IsFieldNumber(FieldNo)
                     then begin
                        Value:=Trim(FExportData.GetFieldValue(FieldNo));
                        if FExportData.IsFieldNull(FieldNo) or (Value='') then begin
                           Value:='0';
                        end;
                        //todo6: добавить параметром печатать ли нули для чисел см. BelarusInfo
                        //if not FExportData.IsFieldNull(FieldNo) and (Value<>'') then begin
                        ...
                        Value:='';
                        if not FExportData.IsFieldNull(FieldNo) then begin
                           Value:=FExportData.GetFieldValue(FieldNo);
                        end;
                        CLABELSST.rw:=Row;
                        CLABELSST.col:=I;
                        CLABELSST.isst:=GetSST(Value, K);
                        CLABELSST.ixfe:=CellAlign[I]+K;
                        Buffer.Write(CLABELSST, SizeOf(TLABELSSTRec));
                     end;
                  end;
                  Inc(Row);
