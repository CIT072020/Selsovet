...
procedure TfmFind2.TreeView1Change(Sender: TObject; Node: TTreeNode);
begin
 //
end;

procedure TfmFind2.TreeView1Collapsed(Sender: TObject; Node: TTreeNode);
var
   Field: TFindFieldItem;
begin
   if (Item.Data=nil) and (Item.Tag=CGroupTag) then begin
...
procedure TfmFind2.TreeView1KeyDown(Sender: TObject; var Key: Word;  Shift: TShiftState);
begin
   case Key of
      VK_LEFT: Key:=VK_UP;
      VK_RIGHT: Key:=VK_DOWN;
...
procedure TfmFind2.TreeView1DblClick(Sender: TObject);
var
   Node: TTreeNode; //TElTreeItem;
begin
   Node:=TreeView1.Selected; // ItemFocused;
   if (Node<>nil) and (TMyTreeNode(Node).Tag=CGroupTag) then begin
      if Node.Expanded then begin
         Node.Collapse(False);
      end
      else begin
...
procedure TfmFind2.TreeView1Expanded(Sender: TObject; Node: TTreeNode);
var
   Field: TFindFieldItem;
begin
   if (Node.Data=nil) and (TMyTreeNode(Node).Tag=CGroupTag) then begin
      Node.ImageIndex:=II_OpenGroup;
      cbGroups.Invalidate;
      cbGroups.Update;
   end;
   if (Node.Data<>nil) then begin
