...
uses Windows, Messages, Classes,Forms, Controls, SysUtils, Variants, Contnrs, db, NativeXML, dbFunc, SasaIniFile, RKP, Math, mMD5,
     TasksEx, AsyncCalls,
     synacode, synachar, mimeinln, blcksock, pop3send,  // Synapse
...
type
  ...
  public
    ...
    function OrgIsDemand(sOrg:String):Boolean;
    ...
    function EnabledCOC(lShow:Boolean):Boolean;
    ...
    procedure SetAutoCheckCountMail;
...
//    function ReceiveMailTest:Boolean;
    ...
    function CreatePOP3_Syna:TPOP3Send;
    function CheckMail_Syna:Boolean;
    procedure Terminate_MailThread(Sender : TObject);
...
//--------------------------------------------------------------
// организация ПК "Одно окно"  или которая требует письмо "в ответ на"   ( "s_window;org13388;" )
function TSMDO.OrgIsDemand(sOrg:String):Boolean;
begin
  Result:=( Pos(';'+ANSILowerCase(Trim(sOrg))+';', ';'+ANSILowercase(GlobalTask.ParamAsString('LIST_DEMAND'))+';')>0 );
end;
//--------------------------------------------------------------
...
//  Avest.OnWriteLog:=
  if not Avest.IsActive then begin
    ...
    WriteTextLog('библиотека ЭЦП Avest ('+NAME_AVEST_DLL+') уже загружена');
  end;
...
//----------------------------------------------------------
function TSMDO.EnabledCOC(lShow:Boolean):Boolean;
var
 IdHTTP: TIdHTTP;
...
begin
  ...
  if FCheckLoadCOC then begin
    ...
    IdSSLIOHandlerSocket.SSLOptions.Method := sslvSSLv3; //sslvTLSv1;
...
//      GlobalTask.SetLastValueAsBoolean('RUN_CHECK_COC',true);
      if EnabledCOC(lShow) then begin
...
//            else Result:=Avest.SMDORefreshCOC(FUrlCOC);     // без проверки
        nRes:=Avest.SMDORefreshCOC2(FUrlCOC,sE,lAllSoob,lShow);     // раздельно по каждому URL
      end else begin
        sErr:=FLastError;
      end;
...
//--------------------------------------------------------------
procedure TSMDO.SetAutoCheckCountMail;
begin
  if fmMain<>nil then begin
    try
      fmMain.TimerNewPost.Enabled:=false;
      if SMDOEnabled and (SMDO.FAutoCheck>0) and (SMDO.FTypeCheckMail=TYPECHECK_COUNTMAIL) and SMDO.FPostEnabled then begin
        fmMain.TimerNewPost.Interval:=SMDO.FAutoCheck*1000*60;
        fmMain.TimerNewPost.Enabled:=true;
        WriteTextLog('Включена автоматическая проверка наличия почты '+IntToStr(SMDO.FAutoCheck)+'мин.');
      end;
    except
    end;
  end;
...
//PATH_AVESTDLL
  ...
  //0(NONE)-не проверять  1(COUNTMAIL)-проверять наличие почты  2(RECEIVEMAIL_G),3(RECEIVEMAIL)-получать почту
  FTypeCheckMail:=Ini.ReadInteger('POST', 'TYPE_CHECK_MAIL', TYPECHECK_RECEIVEMAIL_G);
  FAutoCheck:=Ini.ReadInteger('POST', 'AUTOCHECK', 0);
  if FAutoCheck=0
    then FTypeCheckMail:=TYPECHECK_NONE;
  if (FAutoCheck>0) and (FAutoCheck<10)
    then FAutoCheck:=10;  // минимально допустимое значение !!!
...
//              IdPop.LastCmdResult.Text.SaveToFile('retrive');
...
var
  ...
  sDolg, sErr, sNameFile,sNameRef, sFile:String;
  nErr:DWORD;
begin
  List:=TList.Create;
  ListS:=TList.Create;
  ...
  //              nErr:=Avest.SMDOVerify(sFile, slSign, true);
                ...
                if (sErr='') then begin
                  ...
                  for j:=0 to slSign.Count-1 do begin  // информация по всем ЭЦП
                    ...
                    sDolg:=Avest.GetAttrSign(j,'dolg')+' ';
                    s:=Trim(sDolg+Avest.GetAttrSign(j,'fio')+', '+Avest.GetAttrSign(j,'signdate')+', '+Avest.GetAttrSign(j,'org'));
...
//            fmMain.CheckVisibleGIF(true,'Отправка почты',false);
        ...
        try
          ...
          for i:=0 to slMsg.Count-1 do begin
            ...
            if dmBase.SMDOPost.Locate('ID', slMsg.Strings[i], []) then begin
              ...
              WriteTextLog('-['+IntToStr(i)+']'+StringOfChar('-',40));
      ...
      //    att:=TIdAttachment.Create(idMessage1.MessageParts,'C:\1.txt');
                ...
                if not lUvd then WriteTextLog('отправка сообщения');
                try
                  idSmtp.Send(Msg);
                except
...
//---------------------------------------------
function TSMDO.IsMyOrgCIT:Boolean;
begin
  Result:=MySameText(getKodSMDO,'ORG1466');
end;
//---------------------------------------------
// ID моей системы
...
function TSMDO.CreatePOP3_Syna:TPOP3Send;
begin
  Result:=TPOP3Send.Create;
  Result.TargetHost:=FPopServer;
  Result.TargetPort:=IntToStr(FPopPort);
  Result.UserName:=FUserName;
  Result.Password:=FPassword;
  Result.AuthType:=POP3AuthLogin;
  Result.TimeOut:=15000;
  Result.FullSSL:=FSSL;
  Result.AutoTLS:=FSSL;
end;

function TSMDO.CheckMail_Syna:Boolean;
var
  MThread: TMailThread;
  n:Integer;
begin
  Result:=false;
  WriteTextLog('>>>Проверка почты по таймингу');
  if FRun then begin
    WriteTextLog('Операция недоступна Run=true');
    exit;
  end;
  FRun:=true; // !!!  см.ниже
  if dmBase._LockTypeObj(_TypeObj_SMDOPost, '',false) then begin
    Result:=true;
    MThread:=TMailThread.Create(true);
    MThread.OnTerminate:=Terminate_MailThread;  // работает в основном потоке см.ниже
    MThread.FreeOnTerminate:=true;
    MThread.Priority:=tpLower;
    MThread.Operation:=0;
    MThread.Resume;
  end else begin
    WriteTextLog('Невозможна блокировка');
  end;
end;
//--------------------------------------------------------------------
procedure TSMDO.Terminate_MailThread(Sender : TObject);
var
  n:Integer;
  s:String;
begin
  dmBase._UnLockTypeObj(_TypeObj_SMDOPost);  // !!!  см.выше
  FRun:=false; // !!! см.выше
  n:=TMailThread(Sender).FCount;
  if n>0 then begin
    s:=IntToStr(n);
    fmMain.imgNewPost.Animate:=true;
    fmMain.imgNewPost.Visible:=true;
    if Length(s)<3 then s:=' '+s;
    fmMain.lbNewPost.Caption:=s;
    fmMain.TBToolWindowE.Visible:=true;
    if not Application.Active
      then FlashTrayApp;
    WriteTextLog('>>>'+s);
  end else begin
    fmMain.TBToolWindowE.Visible:=false;
    fmMain.imgNewPost.Visible:=false;
    WriteTextLog('>>>'+TMailThread(Sender).FError);
  end;
//  ShowMessage(inttostr(TMailThread(Sender).FCount)+'  '+TMailThread(Sender).FError);
end;
//--------------------------------------------------------------------
procedure TMailThread.Execute;
begin
  if Operation=0 //MAIL_CHECK
    then FCount:=CheckMail
    else ReceiveMail;
end;
//---------------------------------------------------------------------
function TMailThread.CheckMail:Integer;
var
  s:String;
  i,n:Integer;
  ds:TDataSet;
...
begin
  Result:=-1;
  PO3Client:=SMDO.CreatePOP3_Syna;
  if PO3Client.Login then begin
    PO3Client.Uidl(0);
    if PO3Client.ResultCode=1 then begin
      ds:=dbOpen('BaseProp','','');
      if ds<>nil then begin
        Result:=0;
        for i:=0 to PO3Client.FullResult.Count-1 do begin
          n:=Pos(' ',PO3Client.FullResult.Strings[i]);
          SetLength(arr,3);
          arr[0]:=_TypeObj_UID;
          arr[1]:='UID';
          arr[2]:=Copy(PO3Client.FullResult.Strings[i],n+1,300);
          if not dbLocate(ds,'TYPEOBJ;POKAZ;VALUE',arr,'') then begin
            Result:=Result+1;
          end;
        end;
        dbClose(ds);
      ...
      FError:='Ошибка UID: '+PO3Client.ResultString;
    end;
    PO3Client.Logout;
  end else begin
    FError:=Inttostr(PO3Client.Sock.LastError)+': '+PO3Client.Sock.LastErrorDesc;
  end;
  PO3Client.free;
end;
//---------------------------------------------------------------------
function TMailThread.ReceiveMail:Integer;
begin
  Result:=0;
end;
//---------------------------------------------------------------------
procedure TMailThread.SetOperation(const Value: Integer);
begin
  FOperation := Value;
end;
